...
class M_monitoring extends CI_Model {
	...
	public function get_delivery($id){
		...
		$SQL = "SELECT A.WK_TRUCKIN, A.OPERATOR_T, A.WK_CHASSIS, A.OPERATOR_O, A.WK_INSPECT, A.OPERATOR_I, A.WK_GATEOUT, A.OPERATOR_G, A.NO_SEAL, B.KONDISI
				...
				WHERE A.NO_CONT='$id2' AND C.NO_DOK = '$id3' AND C.FL_BIL = 'DONE'";
	...
	public function get_data($act, $id){
		...
		if($act == 'operation'){
			...
			// echo $id1.' '.$id2; die();
			$data = "SELECT A.NO_SPK,B.NO_CONT AS 'KONTAINER', D.CALL_SIGN, D.NM_ANGKUT AS 'NM_KAPAL', D.NO_VOY_FLIGHT AS 'NO_VOY', C.JNS_CONT, C.UK_CONT AS 'UKR_CONT', concat(C.KD_CONT_TIPE, ' ', ifnull(F.STATUS_DG, '')) as 'KD_CONT_TIPE', F.ISO_CODE, F.IMO, D.TGL_TIBA, C.WK_IN, G.NAMA, A.NO_DOK AS 'DOKUMEN', A.TGL_DOK AS 'TANGGAL', E.CONSIGNEE, C.WK_OUT
						FROM t_spk A
						LEFT JOIN t_spk_cont B ON B.ID = A.ID
						LEFT JOIN (SELECT DISTINCT ID, NO_CONT, JNS_CONT, UK_CONT, KD_CONT_TIPE, ISO_CODE, WK_IN, WK_OUT FROM t_cocostscont ORDER BY ID DESC) C ON B.NO_CONT = C.NO_CONT
			...
			if($type == "excel"){
				if($act=="penarikan"){
					...
					if ($status != "" && $behandle_start != "" && $behandle_end != "" && $pickup_start != "" && $pickup_end != "") {
						$addsql .= " AND B.STATUS_CONT ='450' AND H.W_BEHANDLE BETWEEN '$behandle_start' AND '$behandle_end' AND H.W_PICKUP BETWEEN '$pickup_start' AND '$pickup_end' ORDER BY H.W_BEHANDLE DESC";
					}else if($behandle_start != "" && $behandle_end != "" && $pickup_start != "" && $pickup_end != ""){
						$addsql .= " AND H.W_BEHANDLE BETWEEN '$behandle_start' AND '$behandle_end' AND H.W_PICKUP BETWEEN '$pickup_start' AND '$pickup_end' ORDER BY H.W_BEHANDLE DESC";
					}else if($behandle_start != "" && $behandle_end != ""){
						$addsql .=" AND H.W_BEHANDLE BETWEEN '$behandle_start' AND '$behandle_end' ORDER BY H.W_BEHANDLE DESC";
					}else if($pickup_start != "" && $pickup_end != ""){
						$addsql .=" AND H.W_PICKUP BETWEEN '$pickup_start' AND '$pickup_end' ORDER BY H.W_BEHANDLE DESC";
					}else if($behandle_start != ""){
						$addsql .=" AND H.W_BEHANDLE >= '$behandle_start' ORDER BY H.W_BEHANDLE DESC";
					}else if($behandle_end != ""){
						$addsql .=" AND H.W_BEHANDLE <= '$behandle_end' ORDER BY H.W_BEHANDLE DESC";
					}else if($pickup_start != ""){
						$addsql .=" AND H.W_PICKUP >= '$pickup_start' ORDER BY H.W_BEHANDLE DESC";
					}else if( $pickup_end != ""){
						$addsql .=" AND H.W_PICKUP <= '$pickup_end' ORDER BY H.W_BEHANDLE DESC";
					}else if($status != ""){
						$addsql .=" AND B.STATUS_CONT ='450' ORDER BY H.W_BEHANDLE DESC";
					}else{
						$addsql .=" ORDER BY H.W_BEHANDLE DESC";
					}
					...
					$SQL1 = "SELECT 
						...
						FROM t_spk A
						INNER JOIN t_spk_cont B ON A.ID = B.ID
						LEFT JOIN (SELECT ID, NO_CONT, JNS_CONT, ISO_CODE, KD_CONT_TIPE AS 'TIPE_CONT', WK_IN AS 'STACKING', WK_OUT AS 'GATEOUT' FROM t_cocostscont GROUP BY NO_CONT ORDER BY ID DESC) C ON B.NO_CONT = C.NO_CONT
						LEFT JOIN t_cocostshdr D ON C.ID = D.ID
						INNER JOIN reff_kode_dok_bc E ON A.JNS_DOK = E.ID
						LEFT JOIN (SELECT ID, NO_CONT, IMO FROM t_request_cont ORDER BY ID DESC) F ON B.NO_CONT = F.NO_CONT
						LEFT JOIN t_request G ON F.ID = G.ID
					...
					if($result){
						foreach($SQL->result_array() as $row){
							...
							->setCellValue('R'.$rec,$row["STACKING"])
							->setCellValue('S'.$rec,$row["GATEOUT"])
					...
					$SQL = "SELECT A.ID, A.NO_SPK AS 'NO_SPK', B.NO_CONT AS 'KONTAINER', B.UKR_CONT AS 'SIZE', 'I' AS CLASS, D.NM_ANGKUT AS 'VOYAGE', D.NO_VOY_FLIGHT AS 'NO_VOYAGE', CASE WHEN C.STATUS_CONT ='F' THEN 'FL' ELSE 'MT' END AS 'STATUS_KONTAINER', C.ISO_CODE AS 'ISO_CODE', C.TIPE_CONT AS 'TIPE_KONTAINER', F.IMO, E.NAMA AS 'JENIS_DOKUMEN', A.NO_DOK AS 'NO_DOKUMEN', A.TGL_DOK AS 'TGL_DOKUMEN', G.WK_SEND AS 'REQUEST_GATEPASS', H.W_BEHANDLE AS 'BEHANDLE_IN', I.RESPON AS 'RESPON_PKB', I.WK_RESPON, CONCAT('BEHANDLE ',I.JNS_KEGIATAN) AS 'JENIS_KEGIATAN', A.CONSIGNEE AS 'CUSTOMERS'
						FROM t_spk A
						INNER JOIN t_spk_cont B ON A.ID = B.ID
						LEFT JOIN (SELECT ID, NO_CONT, JNS_CONT AS 'STATUS_CONT', ISO_CODE, KD_CONT_TIPE AS 'TIPE_CONT', WK_IN AS 'STACKING' FROM t_cocostscont GROUP BY NO_CONT ORDER BY ID DESC) C ON B.NO_CONT = C.NO_CONT
						LEFT JOIN t_cocostshdr D ON C.ID = D.ID
					...
					$this->newphpexcel->mergecell(array(array('C1','I1'),array('C2','I2'),array('C3','I3'),array('V1','V3'),array('W1','W3'),array('X1','AC1'),array('AJ1','AK1'),array('X2','Y2'),array('Z2','AA2'),array('AB2','AC2'),array('AD1','AD3'),array('AE1','AE3'),array('V7','W7'),array('K2','K4'),array('L2','L4'),array('M2','R2'),array('M3','N3'),array('O3','P3'),array('Q3','R3'),array('S2','S4'),array('T2','T4')), FALSE);
					$this->newphpexcel->setActiveSheetIndex(0)->setCellValue('C1', 'REPORT RESPON PKB');
					$this->newphpexcel->setActiveSheetIndex(0)->setCellValue('C2', 'COMMON GATE IPC TPK');
					$this->newphpexcel->setActiveSheetIndex(0)->setCellValue('C3', $tgl_awal .' - '. $tgl_akhir);
					$this->newphpexcel->width(array(array('A',5),array('B',20),array('C',20),array('D',10),array('E',20),array('F',20),array('G',20),array('H',15),array('I',15),array('J',20),array('K',15),array('L',15),array('M',25),array('N',25),array('O',25),array('P',30),array('Q',45),array('R',40),array('S',20),array('T',40)));
					...
					$this->newphpexcel->headings(array('A6','B6','C6','D6','E6','F6','G6','H6','I6','J6','K6','L6','M6','N6','O6','P6','Q6','R6','S6','T6','V1','V2','V3','W1','W2','W3','X1','AD1','AD2','AD3','AE1','AE2','AE3','V7','X2','Y2','Z2','AA2','AB2','AC2','X3','Y3','Z3','AA3','AB3','AC3','X7','Y7','Z7','AA7','AB7','AC7','AD7','AE7'));
					...
					$SQL = "SELECT A.NO_SPK AS 'NO_SPK', B.NO_CONT AS 'KONTAINER', B.UKR_CONT AS 'SIZE', 'I' AS CLASS, E.NM_ANGKUT AS 'VOYAGE', E.NO_VOY_FLIGHT AS 'NO_VOYAGE', CASE WHEN D.JNS_CONT ='F' THEN 'FL' ELSE 'MT' END AS 'STATUS_KONTAINER', D.KD_CONT_TIPE AS 'TIPE_KONTAINER', D.ISO_CODE AS 'ISO_CODE', I.JNS_DOK AS 'JENIS_DOKUMEN', I.NO_DOK AS 'NO_DOKUMEN', I.TGL_DOK AS 'TGL_DOKUMEN', I.WK_REK AS 'GATEPASS_TERBIT', H.W_BEHANDLE AS 'BEHANDLE_IN', I.RESPON AS 'RESPON_PKB', J.WK_STATUS AS 'TGL_MARSHALLING', J.LOKASI_AKHIR AS 'LOKASI', CONCAT('BEHANDLE ',I.JNS_KEGIATAN) AS 'JENIS_KEGIATAN', A.CONSIGNEE AS 'CUSTOMERS'
						FROM t_spk A
						...
						INNER JOIN (SELECT A.NO_SPK, B.NO_CONT, A.W_PICKUP, W_BEHANDLE, B.KONDISI_CONT, B.NO_SEAL, B.ROOM FROM t_op_pickup A INNER JOIN t_op_behandlein B ON A.NO_SPK = B.NO_SPK) H ON A.NO_SPK = H.NO_SPK AND B.NO_CONT = H.NO_CONT
						INNER JOIN (SELECT ID, NO_CONT, JNS_DOK,NO_DOK, TGL_DOK, RESPON, WK_RESPON, JNS_KEGIATAN, WK_REK FROM t_gatepass WHERE JNS_KEGIATAN IN ('1','2') GROUP BY NO_CONT,JNS_KEGIATAN ORDER BY ID DESC) I ON B.NO_CONT = I.NO_CONT
						INNER JOIN (SELECT NO_SPK, NO_CONT, NO_GATEPASS, LOKASI_AKHIR, WK_STATUS FROM t_job_slip WHERE LOKASI_AKHIR LIKE 'CIC%' GROUP BY NO_CONT ORDER BY ID_JOB_SLIP DESC) J ON A.NO_SPK = J.NO_SPK AND B.NO_CONT = J.NO_CONT
						WHERE 1 = 1".$addsql;
					...
					if ($TglSelesaiPeriksaAwal != '' && $TglSelesaiPeriksaAkhir != '') {
						$addsql .= " AND J.FINISH_INSP BETWEEN '$TglSelesaiPeriksaAwal' AND '$TglSelesaiPeriksaAkhir' ORDER BY J.FINISH_INSP DESC";
					}else if ($TglSelesaiPeriksaAwal != ''){
						$addsql .= " AND J.FINISH_INSP >= '$TglSelesaiPeriksaAwal' ORDER BY J.FINISH_INSP DESC";
					}else if($TglSelesaiPeriksaAkhir != ''){
						$addsql .= " AND J.FINISH_INSP <= '$TglSelesaiPeriksaAkhir' ORDER BY J.FINISH_INSP DESC";
					}else{
						$addsql .= " ORDER BY J.FINISH_INSP DESC";
					}
					...
					$TglMarshallingExAwal = $TanggalMarshallingEx[0];
					$TglMarshallingExAkhir = $TanggalMarshallingEx[1];
					
					$addsql = " ";
					
					if ($TglMarshallingExAwal != '' && $TglMarshallingExAkhir != '') {
						$addsql .= " AND L.WK_STATUS BETWEEN '$TglMarshallingExAwal' AND '$TglMarshallingExAkhir' ORDER BY L.WK_STATUS DESC";
					}else if ($TglMarshallingExAwal != ''){
						$addsql .= " AND L.WK_STATUS >= '$TglMarshallingExAwal' ORDER BY L.WK_STATUS DESC";
					}else if($TglMarshallingExAkhir != ''){
						$addsql .= " AND L.WK_STATUS <= '$TglMarshallingExAkhir' ORDER BY L.WK_STATUS DESC";
					}else{
						$addsql .= " ORDER BY L.WK_STATUS DESC";
					}
					...
					$SQL = "SELECT A.NO_SPK AS 'NO_SPK', B.NO_CONT AS 'KONTAINER', 'I' AS CLASS, E.NM_ANGKUT AS 'VOYAGE', E.NO_VOY_FLIGHT AS 'NO_VOYAGE', CASE WHEN D.JNS_CONT ='F' THEN 'FL' ELSE 'MT' END AS 'STATUS_KONTAINER', B.UKR_CONT AS 'SIZE', B.ISO_CODE AS 'ISO_CODE', KD_CONT_TIPE AS 'TIPE_KONTAINER', F.IMO, H.JNS_DOK AS 'JENIS_DOKUMEN', H.NO_DOK AS 'NO_DOKUMEN', H.TGL_DOK AS 'TGL_DOKUMEN', H.WK_REK AS 'TERBIT_GATEPASS', K.WK_STATUS AS 'MARSHALLING_BEHANDLE', K.LOKASI_AKHIR AS 'LOKASI_MARSHALLING',J.START_INSP AS 'START_PEMERIKSAAN', J.FINISH_INSP AS 'FINISH_PEMERIKSAAN', J.NO_SEAL AS 'NEW_SEAL_NUMBER', L.WK_STATUS AS 'MARSHALLING_EX_BEHANDLE', L.LOKASI_AKHIR AS 'LOKASI_MARSHALLING_EX_BEHANDLE', CONCAT('BEHANDLE ',H.JNS_KEGIATAN) AS 'JENIS_KEGIATAN', A.CONSIGNEE AS 'CUSTOMERS'
						FROM t_spk A
						INNER JOIN t_spk_cont B ON A.ID = B.ID
						INNER JOIN reff_kode_dok_bc C ON A.JNS_DOK = C.ID
						LEFT JOIN (SELECT ID, NO_CONT, JNS_CONT, ISO_CODE, KD_CONT_TIPE, WK_IN, WK_OUT FROM t_cocostscont GROUP BY NO_CONT ORDER BY ID DESC) D ON B.NO_CONT = D.NO_CONT
