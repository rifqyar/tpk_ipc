	...
	function generate($table_data = NULL){
				...
				foreach ($this->keycari as $a => $b){
					if($b[2]=="DATERANGE"){
						$out .=	'<div class="form-group form-material">';
							...
							$out .=	'<div class="hint">pencarian data tanggal awal '.strtolower($b[1]).'</div>';
							$out .=	'</div>';
							$out .=	'<div class="col-sm-5">';
							$out .=	'<input type="text" name="form['.$a.'][]" id="'.$a.'2" class="date form-control" value="'.$arrvalcari[$a][1].'" placeholder="TANGGAL AKHIR" tag="'.$b[2].'">';
							$out .=	'<div class="hint">pencarian data tanggal akhir '.strtolower($b[1]).'</div>';
							$out .=	'</div>';
						$out .=	'</div>';
					}else if($b[2]=="DATE"){
						$out .=	'<div class="form-group form-material">';
							...
							$out .=	'<input type="text" name="form['.$a.'][]" id="'.$a.'1" class="date form-control" value="'.$arrvalcari[$a][0].'" placeholder="TANGGAL" tag="'.$b[2].'">';
							$out .=	'<div class="hint">pencarian data tanggal '.strtolower($b[1]).'</div>';
							$out .=	'</div>';							
						$out .=	'</div>';
					}else if($b[2]=="OPTION"){
						$out .=	'<div class="form-group form-material">';
		...
		if(count($this->proses) > 0 && $this->show_menu){
			...
			if($this->tipe_proses=="button"){
				...
				$out.="<button type=\"button\" onclick=\"button_menu('".$this->formid."',this.id)\" id=\"tb_menu".$this->formid.$m."\" formid=\"".$this->formid."\" div=\"".$this->divid."\" met=\"".$b[0]."\" url=\"".$b[1]."\" jml=\"".$b[2]."\" status=\"".$b[3]."\" popup=\"".$b[5]."\" class=\"btn btn-icon btn-primary btn-round waves-effect waves-light waves-round\" data-original-title=\"Edit\" data-toggle=\"tooltip\" title=\"".$a."\"><i class=\"".$b[4]."\" aria-hidden=\"true\" div=\"".$this->divid."\"></i></button>";
				$out.="&nbsp;&nbsp;&nbsp;";
				$m++;
				}
			}else{
		...
		$out .= '<input type="hidden" name="tmpchk'.$this->formid.'" id="tmpchk'.$this->formid.'" value="'.$tmpchk.'" mandatory="yes" readonly>';
		$out .='</form>';
		$out .='</div>';
		$out .='<script>$(function(){$(\'[data-toggle="tooltip"]\').tooltip(); date(\'date\'); })</script>';
		//$out .='<script>$(function(){ $(\'[data-toggle="tooltip"]\').tooltip()})</script>';
		return $out;	
	}
