...
class M_requestGatePass extends CI_Model
{
	...
	public function list_reqGatePass($act, $id)
	{
		...
		$proses = array(
			'SEND MAIL'  => array('MODAL', "RequestGatePass/gatepass/add_paidThrough", '1', '', 'md-email', '', 'list'),
		...
		$this->newtable->search(array(
			array('F.NO_CONT', 'NO. KONTAINER'),
			array('C.ANGKUTNAMA_TPS', 'NAMA KAPAL'),
			array('C.ANGKUTNO_TPS', 'NO. VOYAGE'),
			array('A.NO_DOK', 'NO. DOKUMEN'),
			array(
				'B.NAMA',
				'JENIS DOKUMEN',
				'OPTION',
			...
			array('C.STATUS_MAIL ', 'STATUS PENGIRIMAN', 'OPTION', $arr_sts)
		));
	...
	public function list_reqGatePassBc($act, $id)
	{
		...
		$SQLTEMP1 = "SELECT
										...
										CONCAT(
												...
												'  ',
												CASE C.FL_MANUAL
														WHEN 'Y' THEN '<span class=\"label label-primary\">Input Manual</span>'
														WHEN 'N' THEN '<span class=\"label label-success\">Integrasi</span>'
														ELSE ''
												END,
												'<br>TGL : ',
		...
		$SQLTEMP2 = "SELECT
										...
										CASE A.KD_REQ
												WHEN 'DRAFT' THEN CONCAT(
														'<span class=\"label label-danger\">',
														A.KD_REQ,
														'</span>'
												)
												WHEN 'QUEUED' THEN CONCAT(
														'<span class=\"label label-warning\">',
														A.KD_REQ,
														'</span>'
												)
												WHEN 'SENT' THEN CONCAT(
								...
								INNER JOIN t_permit_hdr C
										...
										AND C.TGL_DAFTAR_PABEAN < DATE(A.TGL_DOK) + INTERVAL 1 DAY
								WHERE A.JNS_DOK <> '83'
									AND C.TGL_DAFTAR_PABEAN >= NOW() - INTERVAL 12 MONTH
								ORDER BY C.TGL_DAFTAR_PABEAN DESC
						"; // AND D.TIPE_CONT != 'RFR'
		$SQLTEMP3 = "SELECT
									A.ID AS ID,
									B.NAMA AS `JENIS DOKUMEN`,
							...
							INNER JOIN t_permit_hdr C
									...
									AND C.TGL_DOK_INOUT < DATE(A.TGL_DOK) + INTERVAL 1 DAY
							WHERE A.JNS_DOK <> '83'
								AND C.TGL_DOK_INOUT >= NOW() - INTERVAL 12 MONTH
							ORDER BY C.TGL_DOK_INOUT DESC
					"; // AND D.TIPE_CONT != 'RFR'
		...
		$proses = array(
			'SEND MAIL'  => array('MODAL', "RequestGatePass/gatepassBc/add_paidThrough_karantina", '1', '', 'md-email', '', 'list'),
			'UPDATE CONSIGNE'  => array('MODAL', "RequestGatePass/gatepassBc/updateconsigne", '1', '', 'md-edit', '', 'list'),
...
WHEN C.FL_MANUAL = 'N' THEN  '<span class=\"label label-success\">Integrasi</span>' END,'<BR>TGL : ', (DATE_FORMAT(C.TG_RESPON,'%d-%m-%Y'))) AS 'DOKUMEN', A.NO_BL_AWB AS 'NO BL', CASE WHEN A.KD_REQ = 'DRAFT' THEN CONCAT('<span class=\"label label-danger\">',(A.KD_REQ),'</SPAN>') WHEN A.KD_REQ = 'QUEUED' THEN CONCAT('<span class=\"label label-warning\">',(A.KD_REQ),'</SPAN>') WHEN A.KD_REQ = 'SENT' THEN CONCAT('<span class=\"label label-success\">',(A.KD_REQ),'</SPAN>') WHEN A.KD_REQ = 'APPROVED' THEN CONCAT('<span class=\"label label- PRIMARY\">',(A.KD_REQ),'</SPAN>') WHEN A.KD_REQ = 'INQUIRY' THEN CONCAT('<span class=\"label label-success\">',(A.KD_REQ),'</SPAN>') ELSE A.RESPONSE_REQ END AS 'STATUS_REQUEST', CASE WHEN C.STATUS_MAIL IS NULL THEN '<span class=\"label label-primary\">Email Belum Di Proses</span>' WHEN C.STATUS_MAIL = 'Email Tidak Terkirim' THEN '<span class=\"label label-danger\">Email Tidak Terkirim</span>' WHEN C.STATUS_MAIL = 'Email Terkirim' THEN '<span class=\"label label-success\">Email Terkirim</span>' END AS 'STATUS_PENGIRIMAN', CASE WHEN C.STATUS = 'N' THEN '<span style=\"color:red;font-weight:bold\">REQUEST</span>' WHEN C.STATUS = 'Y' THEN '<span style=\"color:green;font-weight:bold\">DONE</span>' ELSE 0 END AS 'STATUS',A.RESPONSE_REQ AS RESPONSE, CASE WHEN A.WK_FINISH IS NOT NULL THEN '<span class=\"label label-primary\">SELESAI</span>' ELSE '-' END AS KETERANGAN
		...
		$proses = array(
			'SEND MAIL'  => array('MODAL', "RequestGatePass/gatepass/add_paidThrough_karantina", '1', '', 'md-email', '', 'list'),
			'PRINT EXCEL'  => array('EXCEL', "proces/excel/gatepass", '0', '', 'md-file-text', '', 'menu')
		);
		...
		$this->newtable->search(array(
			...
			array('C.ANGKUTNAMA_TPS', 'NAMA KAPAL'),
			array('C.ANGKUTNO_TPS', 'NO. VOYAGE'),
			array('J.NM_TRADER', 'CUSTOMER'),
			array('A.NO_DOK', 'NO. DOKUMEN'),
			array('F.FL_REEFER', 'TIPE KONTAINER', 'OPTION', $arr_tipe),
			array(
				'B.NAMA',
				'JENIS DOKUMEN',
				'OPTION',
			...
			array('A.KD_REQ', 'STATUS PENGIRIMAN', 'OPTION', $arr_sts2),
			array('C.STATUS_MAIL ', 'STATUS PENGIRIMAN', 'OPTION', $arr_sts)
		));
		$this->newtable->action(site_url() . "/requestGatePass/gatepass");
		if ($check) $this->newtable->detail(array('POPUP', "requestGatePass/gatepass/gate_pass_request_detail"));
