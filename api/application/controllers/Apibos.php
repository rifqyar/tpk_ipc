...
class Apibos extends CI_Controller
{
	...
	public function listdokumen_dll()
	{
		...
		try {
			...
			if ($q > 0) {
				$cekListDokumen = $this->db->query("SELECT COUNT(*) as count from list_dokumens ld where ld.NO_DOK = '$nodok' and ld.TGL_DOK = '$tgdok' and ld.FL_STATUS <> 'X'")->row()->count;
				if ($cekListDokumen > 0) {
					$this->httpres(200, 'success', $q, '');
				} else {
					if ($nodok == '' || $tgdok == '') {
	...
	public function listdokumen_sppmp()
	{
		...
		try {
			...
			if ($q > 0) {
				$cekListDokumen = $this->db->query("SELECT COUNT(*) as count from list_dokumens ld where RIGHT(ld.NO_DOK,6) = '$nodok1' and ld.TGL_DOK = '$tgdok' and ld.FL_STATUS <> 'X'")->row()->count;
				if ($cekListDokumen > 0) {
					$this->httpres(200, 'success', $q, '');
				} else {
					if ($nodok == '' || $tgdok == '') {
	...
	public function billingbhd()
	{
		...
		try {
			...
			if ($cekdetail->num_rows() == 0) {
                ...
                WHERE A.NO_DOK_INOUT = '$nodok' and A.TGL_DOK_INOUT = '$tgdok' AND C.JNS_KEGIATAN !='' AND C.FL_BIL IS NULL
                GROUP BY C.NO_CONT, C.JNS_KEGIATAN");
			}
