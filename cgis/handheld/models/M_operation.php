...
class M_operation extends CI_Model
{
	...
	public function index()
	{
		...
		if ($this->content == "") {
			$this->content = $this->load->view('content/operation/index', '', true);
		}
		$data = array(
			'_title_' => 'BOS',
			'_headers_' => $headers,
	...
	public function cek_tspk($par)
	{ //mencocokan inputan dengan no spk di t_spk
		$queryspk = $this->db->get_where('t_spk', array('NO_SPK' => $par));
		return $queryspk->row_array();
	}

	public function cek_tspkcont($par)
	{ //mencocokan inputan dengan NO cont di t_spk_cont
		$queryspk = $this->db->get_where('t_spk_cont', array('NO_CONT' => $par, 'STATUS_CONT' => '200'));
		return $queryspk->row_array();
	}

	public function cek_tspkcontst($par)
	{ //mencocokan inputan dengan NO cont di t_spk_cont
		$queryspk = $this->db->get_where('t_spk_cont', array('NO_CONT' => $par, 'STATUS_CONT' => '460'));
		return $queryspk->row_array();
	}

	public function cek_tspkcontst2($par)
	{ //mencocokan inputan dengan NO cont di t_spk_cont
		$queryspk = $this->db->query("select * from t_spk_cont where NO_CONT='$par' AND STATUS_CONT IN (450,800,850,870)");
		return $queryspk->row_array();
	}

	public function cek_tspkcontid($parid)
	{ //mencocokan inputan dengan NO cont di t_spk_cont
		$queryspk = $this->db->get_where('t_spk_cont', array('ID' => $parid));
		return $queryspk->row_array();
	}

	public function cek_alltspkcont($parid)
	{ //mencocokan inputan dengan NO cont di t_spk_cont
		$queryspk = $this->db->get_where('t_spk_cont', array('ID' => $parid));
		return $queryspk->result_array();
	}

	public function cek_tspk2($parid)
	{ //mencocokan inputan dengan id di t_spk
		$queryspk = $this->db->get_where('t_spk', array('ID' => $parid));
		return $queryspk->row_array();
	}

	public function cek_tspk3($parid, $parid2)
	{ //mencocokan inputan dengan id di t_spk
		$queryspk = $this->db->query("SELECT distinct a.* from t_spk a JOIN t_spk_cont b ON a.ID = b.ID WHERE a.NO_SPK like '%$parid%' AND b.STATUS_CONT IN (000,100,300)");
		return $queryspk->row_array();
	}

	public function ambilspk()
	{ //mengambil data di behandlein untuk menghitung jumlah NO spk yg sama
		$queryspk = $this->db->get('t_op_behandlein');
		return $queryspk->result_array();
	}

	public function ambiljumspk($id)
	{ //mengambil data di behandlein untuk menghitung jumlah NO spk yg sama
		$queryspk = $this->db->get_where('t_spk_cont', array('STATUS_CONT' => '200', 'ID' => $id));
		return $queryspk->result_array();
	}

	public function ambiljumspkreal($id)
	{ //mengambil data di behandlein untuk menghitung jumlah NO spk yg sama
		$queryspk = $this->db->query("select * from t_spk_cont where ID='$id' AND STATUS_CONT IN ('460')");
		return $queryspk->result_array();
	}

	public function after_pickup($par, $par2, $par3, $par4)
	{ //mengubah KD_STATUS menjadi 200 yaitu status sudah pick up
	...
	public function after_hold($par, $par2)
	{ //mengubah KD_STATUS menjadi 300 yaitu status sudah pick up
		$arrdata = array(
			'KD_STATUS' => '300'
		);
	...
	public function after_realisasi($par, $parc, $pars, $seal)
	{ //mengubah KD_STATUS menjadi 300 yaitu status sudah pick up
		$arrdata = array(
			'STATUS_CONT' => '500',
			'NO_SEAL' => $seal
	...
	public function set_hold()
	{
		...
		$ss = date('y');
		$inputan = "MTI-" . $ss . "/" . $this->input->post('nomerspk');
		...
		if ($SQL_SPK['STATUS_CONT'] == '200') {
			if ($SQL_SPK_LOK['LOKASI'] != $lokasi) {
				if ($SQL_DENAH['DENAH'] == $lokasi) {
					...
					// tes percepatan 09 - 10 - 2023 Ersa
					$spk_percepatan = $SQL_SPK['NO_SPK'];
					$servicepercepatan = "INSERT INTO log_pergerakan_kontainer
						(NO_CONT, NO_SPK, ACTIVITY)
						VALUES('$noCont', '$spk_percepatan', 'BEHANDLEIN');";
					$Query = $this->db->query($servicepercepatan);
	...
	// public function set_job_detail($idJobSlip,$jenisPekerjaan,$alat,$operator){

	// }

	// public function get_job($nocont){
	//  $query = $this->db->query("SELECT ID_JOB_SLIP from t_job_slip WHERE NO_CONT = '$nocont'");
	// 	$queryDetails = $this->db->query($query);
	// 	$data = $queryDetails->row();
	// 	return $data;
	// }


	public function set_inspectionn()
	{
		...
		$lokasi 	= $this->input->post('nolok');
		$noTid		= $this->input->post('trucknya');
		$status		= $this->input->post('optradiostatus');
		$TrukIn 	= $this->input->post('kondisi');
		$konSeal 	= $this->input->post('optradio');
		$UKR_CONT 	  = $this->input->post('ukuran');
		$TIPE_CONT 	  = $this->input->post('tipe');
		$FL_DG 	  = $this->input->post('test');
		$nocont = $this->input->post('nocont');
		$idJobSlip = $this->input->post('idJobSlip');
		$idbehandlein = $this->input->post('idbehandlein');
		$jenisPekerjaan  = $this->input->post('jenisPekerjaan');
		$alat  =  $this->input->post('alat');
		$operator    =   $this->input->post('operator');
		$ipenggunaan    =   $this->input->post('penggunaan1');
		$cekJumlah  = strlen($lokasi);
		$jenis_status = json_decode($status);
		$stakoncoar	= $jenis_status->key1;
		$kd_jenis	= $jenis_status->key2;

		$SQL_TIPEKONT	= $this->db->query("SELECT KD_CONT_TIPE from t_cocostscont tc where tc.NO_CONT = '$noCont' order by tc.ID desc limit 1")->row_array();

		$query = $this->db->query("SELECT ID_JOB_SLIP from t_job_slip WHERE NO_CONT = '$nocont'");
		$SQL = "SELECT a.ID_JOB_SLIP, b.ID,b.NO_SPK,b.NO_CONT  from t_job_slip a
				join t_op_behandlein b on a.NO_SPK =b.NO_SPK and b.NO_CONT =a.NO_CONT WHERE a.ID_JOB_SLIP = '$idJobSlip'";
		$queryDetails = $this->db->query($SQL);
		$queryDetails = $this->db->query($query);
		$data = $queryDetails->row();
		...
		if ($SQL_SPK['STATUS_CONT'] == '200') {
			if (substr($lokasi, 0, 2) == "1B" || substr($lokasi, 0, 2) == "1b" || substr($lokasi, 0, 2) == "YB" || substr($lokasi, 0, 2) == "CI") {
				if ($SQL_SPK_LOK['LOKASI'] != $lokasi) {
					if ($SQL_DENAH['DENAH'] == $lokasi) {
						...
						$this->db->insert('t_op_behandlein', $dataBin);
						$insert_idbhdin = $this->db->insert_id();
						/* AFTER INSPECTION */
						$dataAfInspection = array(
							'ISO_CODE' 			=> $isoCode,
							'ID_FLAT' 			=> $noTid,
						...
						/* Update REQUEST CONT FOR BILLING */
						if ($TIPE_CONT == 'RFR') {
							$this->db->query("UPDATE t_request_cont SET FL_PERBAIKI='Y', KD_CONT_JENIS ='$kd_jenis', UKR_CONT ='$UKR_CONT', STATUS_DG = NULL, TIPE_CONT ='$TIPE_CONT', STATUS_BILLING ='$TIPE_CONT' WHERE ID='$id_request' AND NO_CONT='$noCont'");
						} else if ($FL_DG == '') {
							$this->db->query("UPDATE t_request_cont SET FL_PERBAIKI='Y', KD_CONT_JENIS ='$kd_jenis', UKR_CONT ='$UKR_CONT', STATUS_DG = NULL, STATUS_BILLING ='$TIPE_CONT' WHERE ID='$id_request' AND NO_CONT='$noCont'");
						} else {
							$this->db->query("UPDATE t_request_cont SET FL_PERBAIKI='Y', KD_CONT_JENIS ='$kd_jenis', UKR_CONT ='$UKR_CONT', STATUS_DG ='$FL_DG', STATUS_BILLING ='$TIPE_CONT' WHERE ID='$id_request' AND NO_CONT='$noCont'");
						}
						...
						$this->db->update('t_job_slip', array('NO_DOK' => $SQL_SPK['NO_DOK'], 'NO_GATEPASS' => $SQL_GATEPASS[0]['ID'], 'TGL_GATEPASS' => $SQL_GATEPASS[0]['WK_REK'], 'STATUS' => 'DONE',  'KD_STATUS' => '50', 'WK_STATUS' => date('Y-m-d H:i:s'), 'OPERATOR' => $this->session->userdata('USERLOGIN'), 'STATUS_JOB' => 'Y'));
						$dsimpan = array(
							'ID_JOB_SLIP' => $cekJobSlip['ID_JOB_SLIP'],
							'NO_SPK' => $noSPK,
							'NO_CONT' => $noCont,
							'JENIS' => 'BEHANDLE 1',
							'TYPE_CONT' => $SQL_TIPEKONT['KD_CONT_TIPE'],
							'ID_PEKERJAAN' => $jenisPekerjaan,
							'ID_ALAT' => $alat,
							'OPERATOR' => $operator,
							'I_PENGGUNAAN' => $ipenggunaan,
							'ID_USER' => $operator,
							'WK_STATUS' => date('Y-m-d H:i:s')
						);
						$this->db->insert('t_job_detail', $dsimpan);
						...
						if (count($SQL_GATEPASS) > 0) {
							...
							$this->db->insert('t_job_slip', $arrdata);
							$insert_idjob = $this->db->insert_id();
						}
	...
	// 	}
	// }
	public function new_set_marshalling_cic(
		$idJobSlip,
		$nocont,
		$ukuran,
		$lokasiakhir,
		...
		$arrJobDetail = array(
			...
			'JENIS' => 'CIC',
			'TYPE_CONT' => $SQL_TIPEKONT['KD_CONT_TIPE'],
			'ID_PEKERJAAN' => $jenispekerjaan1,
			'ID_ALAT' => $alat1,
			'OPERATOR' => $operator1,
			// 'I_PENGGUNAAN' => $penggunaan1,
			'ID_PEKERJAAN_2' => $jenispekerjaan2,
			'ID_TRUCK' => $truck1,
			'OPERATOR_2' => $operator2,
			// 'I_PENGGUNAAN_2' => $penggunaan2,
			'ID_PEKERJAAN_3' => $jenispekerjaan3,
			'ID_ALAT_3' => $alat3,
			'OPERATOR_3' => $operator3,
			// 'I_PENGGUNAAN_3' => $penggunaan3,
			'ID_USER' => 'admin2',
			'WK_STATUS' => date('Y-m-d H:i:s')
		);
		// print_r($arrJobDetail);
		$this->db->insert('t_job_detail', $arrJobDetail);
		...
		$this->db->where('ID_JOB_SLIP', $ID_JOB);
		$this->db->update('t_job_slip', array('STATUS' => 'DONE', 'STATUS_JOB' => 'Y', 'STATUS_FUMIGASI' => $fumigasi, 'NOTE' => $NOTE, 'KD_STATUS' => '50', 'OPERATOR' => $this->session->userdata('USERLOGIN'), 'WK_STATUS' => date('Y-m-d H:i:s')));
		...
		// buat service percepatan 09 - 10 - 2023 Ersa
		$spk_percepatan = $SQL_JOBSLIP['NO_SPK'];
		$servicepercepatan = "INSERT INTO log_pergerakan_kontainer
		(NO_CONT, NO_SPK, ACTIVITY)
		VALUES('$NO_CONT', '$spk_percepatan', 'MARSHALLING_CIC');";
		$Query = $this->db->query($servicepercepatan);
	...
	public function new_set_marshalling_yard(
		$idJobSlip,
		$nocont,
		$ukuran,
		$lokasiakhir,
		...
		$PKB_RESPON   = $respon;
		$NOTE   	  = $note;
		echo '<br>jenisPekerjaan1  ' . $jenispekerjaan1;
		echo '<br>alat1  ' . $alat1;
		echo '<br>operator1  ' . $operator1;
		echo '<br>jenisPekerjaan2  ' . $jenispekerjaan2;
		echo '<br>truck1  ' . $truck1;
		echo '<br>operator2  ' . $operator2;
		echo '<br>jenisPekerjaan3  ' . $jenispekerjaan3;
		echo '<br>alat3  ' . $alat3;
		echo '<br>operator3  ' . $operator3;
		// die();  

		if ($cekJumlah > 8) {
			$NM_BLOCK = strtoupper(substr($LOK_AKHIR, 0, 8));
			$NM_TIER = strtoupper(substr($LOK_AKHIR, -1));
		} else {
		...
		if ($SQL_PEMERIKSAAN['STATUS'] == 'DONE') {
			...
			$servicepercepatan = "INSERT INTO log_pergerakan_kontainer
					(NO_CONT, NO_SPK, ACTIVITY)
					VALUES('$NO_CONT', '$spk_percepatan', 'MARSHALLING_YARD');";
			$Query = $this->db->query($servicepercepatan);
	...
	public function GetDetailsMarshallingKontainerOld($ID_JOB_SLIP)
	{
		...
		WHERE A.STATUS = 'WAITING' AND A.KD_STATUS='20' AND A.LOKASI_AKHIR LIKE 'CIC%'
			AND C.STATUS_CONT != 900 AND D.FL_ACTIVE = 'Y' AND A.JENIS IN('BEHANDLE 1','BEHANDLE 2') AND A.ID_JOB_SLIP = '$ID_JOB_SLIP'
		GROUP BY A.NO_CONT

		ORDER BY A.ID_JOB_SLIP DESC";
		$queryDetails = $this->db->query($SQL);

		return $queryDetails->row();
	}
	public function GetDetailsMarshallingKontainer($ID_JOB_SLIP)
	{
		$SQL = "SELECT DISTINCT A.ID_JOB_SLIP,A.NO_CONT,C.UKR_CONT,A.LOKASI_AWAL,A.LOKASI_AKHIR,A.TIER_AWAL,A.TIER_AKHIR,A.JENIS,D.RESPON,E.LNSW_NOAJU
		FROM t_job_slip A
		INNER JOIN t_spk B ON A.NO_SPK = B.NO_SPK
		...
		INNER JOIN t_gatepass D ON A.NO_GATEPASS = D.ID
		LEFT JOIN (SELECT * FROM t_ppk_hdr WHERE LNSW_KD_RESPON = '005') E ON D.NO_DOK = E.NO_RESPON AND D.TGL_DOK = E.TG_RESPON
		WHERE A.STATUS = 'WAITING' AND A.KD_STATUS='20'
			AND C.STATUS_CONT != 900 AND D.FL_ACTIVE = 'Y' AND A.JENIS IN('BEHANDLE 1','BEHANDLE 2', 'EX BEHANDLE 1','EX BEHANDLE 2') AND A.ID_JOB_SLIP = '$ID_JOB_SLIP'
		GROUP BY A.NO_CONT
		ORDER BY A.ID_JOB_SLIP DESC";
		$queryDetails = $this->db->query($SQL);
	...
	public function GetTruckDropDownmarshallingkontainer()
	{
		$query = $this->db->query("SELECT * FROM reff_truck;");
		return $query;
	}


	public function GetstatusspkDropDownmarshallingkontainer()
	{
		$query = $this->db->query("SELECT * FROM reff_status_spk;");
		return $query;
	}

	public function GetJspkDropDownmarshallingkontainer()
	{
		$query = $this->db->query("SELECT * FROM m_job_activity;");
		return $query;
	}
	public function GetAlatDropDownmarshallingkontainer()
	{
		$query = $this->db->query("SELECT * FROM t_reff_alat;");
		return $query;
	}
	public function GetOperatorDropDownmarshallingkontainer()
	{
		$query = $this->db->query("SELECT * FROM reff_user WHERE KD_GROUP LIKE '%OPR%';");
		return $query;
	}

	public function InsertDataMarshallingKontainer(
		$idJobSlip,
		$nocont,
		$jenisPekerjaan1,
		$alat1,
		...
		for ($i = 0; $i < count($jenisPekerjaan1); $i++) {
			...
			$dsimpan = array(
				array(
					...
					'OPERATOR' => $operator,
					'I_PENGGUNAAN' => $penggunaan1,
					'ID_PEKERJAAN2' => $jenisPekerjaan2,
					'TRUCK_1' => $truck1,
					'OPERATOR2' => $operator2,
					'I_PENGGUNAAN2' => $penggunaan2,
					'ID_PEKERJAAN3' => $jenisPekerjaan3,
					'ID_ALAT3' => $alat3,
					'OPERATOR3' => $operator3,
					'I_PENGGUNAAN3' => $penggunaan3,
					'ID_USER' => $this->session->userdata('USERLOGIN'),
					'WK_STATUS' => date('Y-m-d H:i:s')
				)
			);
			$this->db->insert_batch('t_job_detail', $dsimpan);
		}
	...
	public function InsertTambahAlatBehandlein($idJobSlip, $jenisPekerjaan, $alat, $operator, $ChedkIdBhndl)
	{

		for ($i = 0; $i < count($jenisPekerjaan); $i++) {
			...
			$dsimpan = array(
				array(
					'ID_JOB_SLIP' => $idJobSlip,
					'ID_BEHANDLE_IN' => $data->ID,
					'ID_PEKERJAAN' => $jenisPekerjaan[$i],
					'ID_ALAT' => $alat[$i],
					'OPERATOR' => $operator[$i]
				)
			);
	...
	public function GetDetailsMarshallingCIC($ID_JOB_SLIP)
	{
		$SQL = "SELECT DISTINCT A.ID_JOB_SLIP,A.NO_CONT,C.UKR_CONT,A.LOKASI_AWAL,A.LOKASI_AKHIR,A.TIER_AWAL,A.TIER_AKHIR,A.JENIS,D.RESPON,E.LNSW_NOAJU
		FROM t_job_slip A
		INNER JOIN t_spk B ON A.NO_SPK = B.NO_SPK
	...
	public function GetstatusspkDropDownmarshallingCIC()
	{
		$query = $this->db->query("SELECT * FROM reff_status_spk ");
		return $query;
	}

	public function GetJspkDropDownmarshallingCIC()
	{
		$query = $this->db->query("SELECT * FROM m_job_activity;");
		return $query;
	}
	public function GetAlatDropDownmarshallingCIC()
	{
		$query = $this->db->query("SELECT * FROM t_reff_alat;");
		return $query;
	}

	public function GetDetailsMarshallingYARD($ID_JOB_SLIP)
	{
		$SQL = "SELECT DISTINCT A.ID_JOB_SLIP,A.NO_CONT,C.UKR_CONT,A.LOKASI_AWAL,A.LOKASI_AKHIR,A.TIER_AWAL,A.TIER_AKHIR,A.JENIS,D.RESPON,E.LNSW_NOAJU
		FROM t_job_slip A
		INNER JOIN t_spk B ON A.NO_SPK = B.NO_SPK
	...
	public function GetstatusspkDropDownmarshallingYARD()
	{
		$query = $this->db->query("SELECT * FROM reff_status_spk ");
		return $query;
	}

	public function GetJspkDropDownmarshallingYARD()
	{
		$query = $this->db->query("SELECT * FROM m_job_activity;");
		return $query;
	}
	public function GetAlatDropDownmarshallingYARD()
	{
		$query = $this->db->query("SELECT * FROM t_reff_alat;");
		return $query;
	}

	public function InsertNewYard() {}

	public function set_marshalling_detail()
	{
		if (isset($_POST['submit'])) {
			redirect('content/operation/detailmarshalling');
			return;
		} else {
	...
	public function set_listcontainer()
	{
		$ID	  		  = $this->input->post('id');
		$NO_CONT 	  = $this->input->post('nomercont');
		$STATUS 	  = $this->input->post('status_tipe');
		$UKR_CONT 	  = $this->input->post('ukuran');
		$TIPE_CONT 	  = $this->input->post('tipe');
		$FL_DG 	  = $this->input->post('test');

		$TIPE_CONT  = strtoupper($TIPE_CONT);

		//echo $STATUS;die();


		$SQL = $this->db->query("SELECT a.ID, a.NO_CONT, b.NO_DOK, b.TGL_DOK, a.UKR_CONT, a.TIPE_CONT, a.KD_STATUS from t_request_cont a inner join t_request b on a.ID = b.ID where a.TIPE_CONT = 'RFR' and a.KD_STATUS ='INQUIRY' and month(b.TGL_DOK) >= 6 and year(b.TGL_DOK) >= 2021 and a.FL_PERBAIKI ='N' and a.ID='$ID' AND a.NO_CONT='$NO_CONT'")->result();

		//var_dump($SQL[0]->ID);die();
		//return $SQL->result_array();

		if ($SQL[0]->ID != '') {
			$data = $this->db->query("UPDATE t_request_cont SET FL_PERBAIKI='Y',  KD_CONT_JENIS='$STATUS', UKR_CONT ='$UKR_CONT', STATUS_BILLING ='$TIPE_CONT', FL_DG ='$FL_DG'  WHERE ID='$ID' AND NO_CONT='$NO_CONT'");
	...
	public function set_holdd()
	{

		$ID	  		  = $_POST['id'];
		$NO_SPK 	  = $_POST['nospk'];
		$NO_CONT 	  = $_POST['nomercont'];
		$NO_DOK 	  = $_POST['nodok'];
		$TGL_DOK 	  = $_POST['tgldok'];
		$JNS_DOK 	  = $_POST['jnsdok'];
		$WARNA 	  	  = $_POST['warna'];

		$NO_DOK  = strtoupper($NO_DOK);

		// echo var_dump($ID);
		// echo var_dump($NO_SPK);
		// echo var_dump($NO_CONT);
		// echo var_dump($NO_DOK);
		// echo var_dump($TGL_DOK);die();

		$cekdokumen = $this->db->query("SELECT * FROM t_spk_cont
		where ID='$ID' AND NO_CONT ='$NO_CONT'")->result();

		// var_dump($cekdokumen[0]->ID);die();
		//return $cekdokumen->result_array();

		if ($cekdokumen[0]->ID != '') {
			$data = $this->db->query("UPDATE t_spk_cont SET FL_HOLD='Y', FL_WARNA_HOLD='$WARNA' WHERE ID='$ID' AND NO_CONT ='$NO_CONT' ");
		} else {
			echo "Error";
		}
	}
	...
	public function set_release()
	{
		...
		$NO_CONT 	  = $this->input->post('nomercont');
		$NO_DOK 	  = $this->input->post('nodok');
		$TGL_DOK 	  = $this->input->post('tgldok');
		$JNS_DOK 	  = $this->input->post('jnsdok');
	...
	public function set_pemeriksaan($operator)
	{
		$NO_CONT = $this->input->post('nomerkon');
		$NO_SEAL = $this->input->post('noseal');
		$idJobSlip = $this->input->post('idJobSlip');
		$noSPK = $this->input->post('nospk');
		$tipecont = $this->input->post('tipecont');
		$alat  =  $this->input->post('alat');
		$operator  = $this->input->post('operator');
		$JOIN = $this->input->post('join');


		// $query = $this->db->query("SELECT ID_JOB_SLIP from t_job_slip WHERE NO_CONT = '$NO_CONT'");
		// $SQL = "SELECT a.ID_JOB_SLIP, b.ID,b.NO_SPK,b.NO_CONT  from t_job_slip a
		// 		join t_op_behandlein b on a.NO_SPK =b.NO_SPK and b.NO_CONT =a.NO_CONT WHERE a.ID_JOB_SLIP = '$idJobSlip'";
		// 		$queryDetails = $this->db->query($SQL);
		...
		if ($SQL_SPK_CONT['NO_CONT'] == $NO_CONT) {
			if ($SQL_SPK_CONT['STATUS_CONT'] == '460') {
				if (count($SQL_PEMERIKSAAN) == 0) {
					...
					$servicepercepatan = "INSERT INTO log_pergerakan_kontainer
					(NO_CONT, NO_SPK, ACTIVITY)
					VALUES('$NO_CONT', '$noSPK', 'START_PERIKSA');";
					$Query = $this->db->query($servicepercepatan);
					...
					$servicepercepatan = "INSERT INTO log_pergerakan_kontainer
					(NO_CONT, NO_SPK, ACTIVITY)
					VALUES('$NO_CONT', '$noSPK', 'FINISH_PERIKSA');";
					$Query = $this->db->query($servicepercepatan);
					...
					// $idjobslip5 = $this->db->query("SELECT * FROM t_job_slip WHERE NO_SPK = '".$SQL_SPK['NO_SPK']."' AND NO_CONT = '".$noCont."' AND NO_DOK = '".$SQL_SPK['NO_DOK']."' AND JENIS = 'PICKUP' ORDER BY ID_JOB_SLIP DESC LIMIT 1")->row_array();

					$idjobslip5 = $this->db->query("SELECT * FROM t_job_slip WHERE NO_CONT = '" . $NO_CONT . "' AND NO_SPK = '" . $SQL_SPK_CONT['NO_SPK'] . "' AND JENIS = 'PICKUP' ORDER BY ID_JOB_SLIP DESC LIMIT 1")->row_array();
					...
					$dsimpan = array(
						'ID_JOB_SLIP' => $idJobSlip5['ID_JOB_SLIP'],
						'JENIS' => 'PEMERIKSAAN',
						'NO_SPK' => $noSPK,
						'NO_CONT' => $NO_CONT,
						'TYPE_CONT' => $tipecont,
	...
	public function getTotalAllDataReportStack($starDate, $endDate)
	{
		...
		if (!empty($starDate) && !empty($endDate)) {
			$this->db->where("tr.TGL_DOK BETWEEN '$starDate' AND '$endDate'");
		}
		$this->db->select('tr.NO_DOK as no,tr.NO_DOK, tr.TGL_DOK, trc.NO_CONT, trc.UKR_CONT, trc.TIPE_CONT, trc.DISCHARGE as WAKTU_MASUK, ts.NO_SPK, tob.W_BEHANDLE, tod.WK_GATEOUT');
		$this->db->from('t_request tr');
		$this->db->join('t_request_cont trc', 'tr.ID = trc.ID');
		$this->db->join('t_spk ts', 'tr.NO_DOK = ts.NO_DOK');
		$this->db->join('t_spk tss', 'tr.TGL_DOK = tss.TGL_DOK');
		$this->db->join('t_op_behandlein tob', 'tob.NO_SPK = ts.NO_SPK');
		$this->db->join('t_op_behandlein tob1', 'tob1.NO_CONT = trc.NO_CONT');
		$this->db->join('t_op_delivery tod ', 'tod.NO_SPK = ts.NO_SPK');
		$this->db->join('t_op_delivery tod1 ', 'tod1.NO_CONT = trc.NO_CONT');
		$this->db->select("COALESCE(tr.NO_DOK, '-') as NO_DOK", FALSE);
		$this->db->select("COALESCE(tr.TGL_DOK, '-') as TGL_DOK", FALSE);
		$this->db->select("COALESCE(trc.NO_CONT, '-') as NO_CONT", FALSE);
		$this->db->select("COALESCE(trc.UKR_CONT, '-') as UKR_CONT", FALSE);
		$this->db->select("COALESCE(trc.TIPE_CONT, '-') as TIPE_CONT", FALSE);
		$this->db->select("COALESCE(trc.DISCHARGE, '-') as WAKTU_MASUK", FALSE);
		$this->db->select("COALESCE(ts.NO_SPK, '-') as NO_SPK", FALSE);
		$this->db->select("COALESCE(tob.W_BEHANDLE, '-') as WAKTU_BEHADNLE_IN", FALSE);
		$this->db->select("COALESCE(tod.WK_GATEOUT, '-') as GATEOUT", FALSE);
		$query = $this->db->count_all_results();

		return $query;
	}

	public function ServerSideGetDataReportStack($starDate, $endDate, $start, $length)
	{
	...
	public function getAllDataReportStack($starDate, $endDate)
	{
		if (!empty($starDate) && !empty($endDate)) {
			$this->db->where("tr.TGL_DOK BETWEEN '$starDate' AND '$endDate'");
		}
		$this->db->select('tr.NO_DOK, tr.TGL_DOK, trc.NO_CONT, trc.UKR_CONT, trc.TIPE_CONT, trc.DISCHARGE as WAKTU_MASUK, ts.NO_SPK, tob.W_BEHANDLE as WAKTU_BEHANDLE_IN, tod.WK_GATEOUT as GATEOUT');
		$this->db->from('t_request tr');
		$this->db->join('t_request_cont trc', 'tr.ID = trc.ID');
		$this->db->join('t_spk ts', 'tr.NO_DOK = ts.NO_DOK');
		$this->db->join('t_spk tss', 'tr.TGL_DOK = tss.TGL_DOK');
		...
		$this->db->select("COALESCE(trc.DISCHARGE, '-') as WAKTU_MASUK", FALSE);
		$this->db->select("COALESCE(ts.NO_SPK, '-') as NO_SPK", FALSE);
		$this->db->select("COALESCE(tob.W_BEHANDLE, '-') as WAKTU_BEHANDLE_IN", FALSE);
		$this->db->select("COALESCE(tod.WK_GATEOUT, '-') as GATEOUT", FALSE);
	...
	public function ambilmarshkontainer($keyword)
	{
		$SQL = "SELECT DISTINCT A.ID_JOB_SLIP,A.NO_CONT,C.UKR_CONT,A.LOKASI_AWAL,A.LOKASI_AKHIR,A.TIER_AWAL,A.TIER_AKHIR,A.JENIS,D.RESPON
				FROM t_job_slip A
				INNER JOIN t_spk B ON A.NO_SPK = B.NO_SPK
	...
	public function ambildetailkontainer($keyword)
	{
		$SQL = "SELECT DISTINCT A.ID_JOB_SLIP,A.NO_CONT,C.UKR_CONT,A.LOKASI_AWAL,A.LOKASI_AKHIR,A.TIER_AWAL,A.TIER_AKHIR,A.JENIS,D.RESPON
				FROM t_job_slip A
				INNER JOIN t_spk B ON A.NO_SPK = B.NO_SPK
	...
	public function getalljobscic()
	{
		...
		INNER JOIN t_gatepass D ON A.NO_GATEPASS = D.ID
		LEFT JOIN (SELECT * FROM t_ppk_hdr WHERE LNSW_KD_RESPON = '005') E ON D.NO_DOK = E.NO_RESPON AND D.TGL_DOK = E.TG_RESPON
		WHERE not exists (select 1 from t_atensi_p2 where NO_CONT = C.NO_CONT and NO_SPK = B.NO_SPK) AND A.STATUS = 'WAITING' AND A.KD_STATUS='20' AND A.LOKASI_AKHIR LIKE 'CIC%'
			AND C.STATUS_CONT != 900 AND D.FL_ACTIVE = 'Y' AND A.JENIS IN('BEHANDLE 1','BEHANDLE 2', 'EX BEHANDLE 1','EX BEHANDLE 2')
		GROUP BY A.NO_CONT
		ORDER BY A.ID_JOB_SLIP DESC";
		$queryspk = $this->db->query($SQL);
	...
	public function getalljobskontainer()
	{
		$var = 'CIC';
		$SQL = "SELECT DISTINCT A.ID_JOB_SLIP,A.NO_CONT,C.UKR_CONT,A.LOKASI_AWAL,A.LOKASI_AKHIR,A.TIER_AWAL,A.TIER_AKHIR,A.JENIS,D.RESPON,E.LNSW_NOAJU
		FROM t_job_slip A
	...
	public function getalldetailkontainer()
	{
		$var = 'CIC';
		$SQL = "SELECT DISTINCT A.ID_JOB_SLIP,A.NO_CONT,C.UKR_CONT,A.LOKASI_AWAL,A.LOKASI_AKHIR,A.TIER_AWAL,A.TIER_AKHIR,A.JENIS,D.RESPON,E.LNSW_NOAJU
		FROM t_job_slip A
	...
	public function datajumlahmarshallingkontainer()
	{
		$queryspk = $this->db->query("SELECT DISTINCT A.ID_JOB_SLIP,A.NO_CONT,A.LOKASI_AWAL,A.LOKASI_AKHIR
			FROM t_job_slip A
			INNER JOIN t_spk B ON A.NO_SPK = B.NO_SPK
	...
	//       // $this->db->select('DISTINCT ID, NAMA');
	//       // 		  ->from('reff_user')
	//       // 		  ->where('KD_GROUP' = 'OPR', )
	//       $query = $this->db->query("SELECT DISTINCT ID, NAMA from reff_user where KD_GROUP = 'OPR' LIKE '%$str%' ORDER BY ID desc");
	//       return $query->result();
	//   }


	public function getOperator2($searchTerm = "")
	{
		//  $this->db->like('NAMA', $searchTerm);
		// $query = $this->db->select("ID,NAMA")->get('reff_user');
		...
		$fetched_records = $this->db->get('reff_user');
	...
	public function searchh($keyword)
	{
		$queryspk = $this->db->query("SELECT A.ID, A.NO_CONT,B.NO_DOK, B.NO_SPK, B.TGL_DOK, A.UKR_CONT,E.TIPE_CONT, CASE WHEN LEFT(A.LOKASI,3) = 'CIC' THEN CONCAT(A.LOKASI,'0',A.TIER) ELSE CONCAT(A.LOKASI,'0',A.TIER) END AS 'LOKASI', A.TIER, A.ROOM, A.STATUS_CONT, A.NO_SEAL, A.ISO_CODE, A.ID_FLAT, A.FL_GATEPASS, E.ID as 'ID_REQUEST', E.STATUS_BILLING, E.FL_DG, E.IMO
		FROM t_spk_cont A join t_spk B on B.ID = A.ID left join (select d.ID,c.NO_CONT, d.NO_DOK, d.TGL_DOK, c.TIPE_CONT, c.STATUS_BILLING, c.FL_PERBAIKI, c.FL_DG, c.IMO from t_request_cont c 
		inner join t_request d on c.ID = d.ID ) E on E.NO_CONT = A.NO_CONT and E.NO_DOK = B.NO_DOK and E.TGL_DOK = B.TGL_DOK 
		WHERE A.NO_CONT LIKE '%$keyword%' AND A.STATUS_CONT IN (200) and E.FL_PERBAIKI ='N'");
		return $queryspk->row_array();
	}

	public function search_hold($keyword)
	{
		$queryspk = $this->db->query(" SELECT d.ID, d.NO_SPK, c.NO_CONT, d.TGL_DOK, d.NO_DOK, a.NAMA AS 'JNS_DOK', (case when c.FL_HOLD = 'Y' then 'HOLD' END) AS 'KETERANGAN' from t_spk d join t_spk_cont c on c.ID = d.ID
    	left join reff_kode_dok_bc a on a.ID = d.JNS_DOK
		WHERE c.NO_CONT LIKE '%$keyword%' and c.FL_HOLD ='N' and c.STATUS_CONT !=900 ");
		return $queryspk->row_array();
	}
	...
	public function search_realease($keyword)
	{
		$queryspk = $this->db->query("SELECT d.ID, d.NO_SPK, c.NO_CONT, d.TGL_DOK, d.NO_DOK, a.NAMA AS 'JNS_DOK', (case when c.FL_HOLD = 'Y' then 'HOLD' END) AS 'KETERANGAN', (case when c.FL_WARNA_HOLD = 'N' then 'PUTIH' 
		when c.FL_WARNA_HOLD = 'M' then 'MERAH' 
		when c.FL_WARNA_HOLD = 'T' then 'TIMAH' END) AS 'WARNA' 
		from t_spk d join t_spk_cont c on c.ID = d.ID
    	left join reff_kode_dok_bc a on a.ID = d.JNS_DOK 
		where c.NO_CONT LIKE '%$keyword%' and c.FL_HOLD ='Y' and c.STATUS_CONT !=900 ");
		return $queryspk;
	}

	public function realese()
	{
		$SQL = $this->db->query("SELECT d.ID, d.NO_SPK, c.NO_CONT, d.TGL_DOK, d.NO_DOK, a.NAMA AS 'JNS_DOK', (case when c.FL_HOLD = 'Y' then 'HOLD' END) AS 'KETERANGAN', (case when c.FL_WARNA_HOLD = 'N' then 'PUTIH' 
		when c.FL_WARNA_HOLD = 'M' then 'MERAH' 
		when c.FL_WARNA_HOLD = 'T' then 'TIMAH' END) AS 'WARNA' 
		from t_spk d join t_spk_cont c on c.ID = d.ID
    	left join reff_kode_dok_bc a on a.ID = d.JNS_DOK 
		where c.FL_HOLD ='Y' and c.STATUS_CONT !=900");
		//var_dump($SQL);die();
		return $SQL->result_array();
	}
	...
	public function searchb23($keyword)
	{
		$queryspk = $this->db->query("select b.*, a.NO_DOK,a.TGL_DOK, E.ID as ID_REQUEST,E.UKR_CONT,E.TIPE_CONT, E.STATUS_BILLING, E.FL_PERBAIKI from t_spk a join t_spk_cont b on a.ID = b.ID 
		left join (select d.ID,c.NO_CONT, d.NO_DOK, d.TGL_DOK,c.UKR_CONT, c.TIPE_CONT,c.STATUS_BILLING, c.FL_PERBAIKI from t_request_cont c 
		inner join t_request d on c.ID = d.ID ) E on E.NO_CONT = b.NO_CONT and E.NO_DOK = a.NO_DOK and E.TGL_DOK = a.TGL_DOK 
		where b.NO_CONT LIKE '%$keyword%' AND b.STATUS_CONT='200' and E.FL_PERBAIKI ='N'");
		return $queryspk;
	}
	...
	{
		$queryspk = $this->db->query("select A.*, B.NO_SPK, C.TIPE_CONT, E.ID_JOB_SLIP FROM t_spk_cont A INNER JOIN t_spk B ON A.ID=B.ID INNER JOIN t_request_cont C ON A.NO_CONT=C.NO_CONT INNER JOIN t_op_behandlein D ON A.NO_CONT=D.NO_CONT INNER JOIN t_job_slip E ON D.NO_SPK=E.NO_SPK WHERE A.NO_CONT LIKE UPPER('%$keyword%') AND A.STATUS_CONT IN('460') ORDER BY ID DESC LIMIT 1");
		return $queryspk->result_array();
	...
	{
		$SQL = $this->db->query("SELECT a.ID, a.NO_CONT,a.KD_CONT_JENIS, CASE WHEN a.KD_CONT_JENIS ='F' THEN 'FULL' ELSE 'EMPTY' END AS 'STATUS', a.UKR_CONT, a.TIPE_CONT, a.KD_STATUS, a.FL_REEFER,a.FL_DG, a.IMO, a.STATUS_BILLING from t_request_cont a 
		inner join t_request b on a.ID = b.ID where a.KD_STATUS ='INQUIRY' and month(b.TGL_DOK) >= 6 
		and year(b.TGL_DOK) >= 2021 and a.FL_PERBAIKI ='N' and A.NO_CONT in ('EMCU8042967','DFOU6116928') limit 2");
		//var_dump($SQL);die();
		return $SQL->result_array();
	}
	...
	{
		$SQL = $this->db->query("SELECT C.ID, C.NO_SPK, A.NO_CONT, B.NO_DOK, B.TGL_DOK, D.NAMA as 'JNS_DOK',
		(case when C.KD_STATUS = '300' then 'HOLD' END) AS 'KETERANGAN' from t_request_cont A 
		inner join t_request B on A.ID = B.ID 
		left join (select d.ID, d.NO_SPK, c.NO_CONT, d.TGL_DOK, d.NO_DOK, d.JNS_DOK, d.KD_STATUS from t_spk d join t_spk_cont c on c.ID = d.ID) C on C.NO_CONT = A.NO_CONT and C.NO_DOK = B.NO_DOK and C.TGL_DOK = B.TGL_DOK
		left join reff_kode_dok_bc D on d.ID = B.JNS_DOK 
		where C.KD_STATUS ='300'");
		//var_dump($SQL);die();
		return $SQL->result_array();
	}
	...
	public function tampiljobactivity()
	{
		$hasil = $this->db->query("select * from m_job_activity order by ID asc");
		return $hasil;
	}
	...
	{
		$data = array(
			'jenis_pekerjaan'     => $this->input->post('jenispekerja')
		);
		$this->db->insert('m_job_activity', $data);
	}
	...
	public function set_new_marshalling()
	{
		$ID_JOB 	  = $this->input->post('jobs');
		$NO_CONT 	  = $this->input->post('nocont');
		$UKR_CONT 	  = $this->input->post('ukrcont');
		$LOK_AKHIR 	  = $this->input->post('lokak');
		$JNS_KEGIATAN = $this->input->post('job');
		$STATUS_CONTAINER = $this->input->post('statuscont');
		$cekJumlah  = strlen($LOK_AKHIR);
	...
	public function GetDataDashboardMti($starDate, $endDate)
	{

		$query = $this->db->select('tsc.NO_CONT as NO_CONT,
			...
			->from('t_spk ts')
			->join('t_spk_cont tsc ', ' ts.ID = tsc.ID')
			->join('t_op_behandlein tob ', ' ts.NO_SPK = tob.NO_SPK')
			->join('t_op_inspection toi ', ' toi.NO_CONT = tsc.NO_CONT and toi.NO_SPK = ts.NO_SPK ')
			->join('t_op_delivery tod ', ' tod.NO_CONT = tsc.NO_CONT and tod.NO_SPK = ts.NO_SPK ')
			->join('t_gatepass tg ', ' tg.NO_CONT = tsc.NO_CONT and tg.NO_SPK = ts.NO_SPK')
			->join('reff_kode_dok_bc rkdb ', ' ts.JNS_DOK = rkdb.ID')
			->join('req_behandle_hdr rbh ', ' rbh.NO_DOK = ts.NO_DOK and rbh.TGL_DOK = ts.TGL_DOK')
			->join('req_delivery_hdr rdh ', ' rdh.NO_DOK = tg.NO_DOK and rdh.TGL_DOK = tg.TGL_DOK')
			->where('tod.WK_GATEOUT >=', $starDate)
			->where('tod.WK_GATEOUT <=', $endDate);
		//->limit(10);
		$results = $query->get();
		return $results->result_array();
	}

	public function SearchReExport(string $keyword, string $from)
	{
		$query = "
			SELECT
				tre.NO_DOK,
			...
			) dok_baru on tre.NO_DOK = dok_baru.no_dok and tre.TGL_DOK = dok_baru.TGL_DOK
			where not exists (
					SELECT 1 FROM t_op_strip_stuff where NO_DOK = tre.NO_DOK AND NO_CONT = dok_baru.NO_CONT AND NO_DOK_LAMA = tre.NO_DOK_LAMA AND NO_CONT_LAMA = tre.NO_CONT_LAMA and WK_END_OP is not NULL
				) 
				AND (tre.NO_CONT_LAMA like '%$keyword%' or dok_baru.no_cont like '%$keyword%')
		";
		...
		if ($from == 'search') {
			...
			$query = "SELECT WK_START_OP FROM t_op_strip_stuff toss where toss.NO_CONT like '%$keyword%' or toss.NO_CONT_LAMA like '%$keyword%'";
			$cekStripStuffStart = $this->db->query($query)->row();
			foreach ($data as $key => $value) {
				$value->WK_START_STRIPSTUF = $cekStripStuffStart->WK_START_OP;
			}
	...
	public function ExecStripStuff($postData)
	{
		$no_cont 			= $postData['nomerkon'];
		$no_dok  			= $postData['no_dok'];
		$no_cont_lama	= $postData['no_cont_lama'];
		$no_dok_lama 	=	$postData['no_dok_lama'];

		$return['notif'] = 0;

		$cekStripStuff = $this->db->query("SELECT ID, WK_START_OP, WK_END_OP from t_op_strip_stuff where NO_DOK = ? AND NO_CONT = ? AND NO_DOK_LAMA = ? AND NO_CONT_LAMA = ?", array($no_dok, $no_cont, $no_dok_lama, $no_cont_lama))->row();
		if ($cekStripStuff) {
			if (!empty($cekStripStuff->WK_START_OP) && !empty($cekStripStuff->WK_END_OP)) {
				$return['notif'] = '4';
			} else {
				$currentDate = date('Y-m-d H:i:s');
				$user = $this->session->userdata('USERLOGIN');

				if ($cekStripStuff->WK_END_OP == '' || $cekStripStuff->WK_END_OP == null) {
					$query = "UPDATE t_op_strip_stuff SET WK_END_OP = ?, OP_END = ? where ID = ?";
					$queryParam = array(
						$currentDate,
						$user,
						$cekStripStuff->ID
					...
					$exec = $this->db->query($query, $queryParam);
					if ($exec) {
						$return['notif'] = '5';
					} else {
						$return['notif'] = '9';
			...
			$query = "INSERT INTO tpk_ipc.t_op_strip_stuff (NO_DOK, NO_CONT, NO_DOK_LAMA, NO_CONT_LAMA, WK_START_OP, OP_START) VALUES(?, ?, ?, ?, ?, ?)";
			$paramInsert = array(
				$no_dok,
				$no_cont,
				$no_dok_lama,
			...
			$exec = $this->db->query($query, $paramInsert);
			if ($exec) {
				$return['notif'] = '6';
			} else {
				$return['notif'] = '9';
