...
// $filelog = $CONF['root.dir'] . $method . ".log";
...
if (!$CheckFile) {
    ...
    if (date('H') >= $jam_awal && date('H') <= $jam_akhir) {
		/*$SQL1 = "SELECT DISTINCT A.ID_REQ, A.TGL_REQ, A.JNS_DOK, A.NO_DOK, A.TGL_DOK, A.NM_KAPAL, A.NO_VOY, A.NO_NOTA_DELIVERY, A.TGL_NOTA, A.TGL_STACK AS WK_IN, C.WK_OUT, B.NO_CONT, B.UKR_CONT, B.ISO_CODE, B.`STATUS`, B.TARIF_ID, B.CHARGE
				...
				INNER JOIN t_cocostshdr D ON A.NM_KAPAL = D.NM_ANGKUT AND A.NO_VOY = D.NO_VOY_FLIGHT
			...
			-- LIMIT 100
			";*/
		$SQL1 = "SELECT DISTINCT A.ID_REQ, A.TGL_REQ, A.JNS_DOK, A.NO_DOK, A.TGL_DOK, A.NM_KAPAL, A.NO_VOY, A.NO_NOTA_DELIVERY, A.TGL_NOTA, 
				 D.WK_IN,D.WK_OUT, B.NO_CONT, B.UKR_CONT, B.ISO_CODE, B.`STATUS`, B.TARIF_ID, B.CHARGE 
				 FROM req_delivery_hdr A 
				 INNER JOIN req_delivery_dtl B ON A.ID_REQ = B.ID_REQ
				 INNER JOIN t_cocostshdr C ON A.NM_KAPAL = C.NM_ANGKUT AND C.NO_VOY_FLIGHT = A.NO_VOY
				 INNER JOIN t_cocostscont D ON B.NO_CONT = D.NO_CONT AND C.ID = D.ID AND D.WK_OUT IS NOT NULL AND D.WK_IN IS NOT NULL
				 WHERE SUBSTRING(A.ID_REQ,1,3) = 'DEL' AND B.NO_CONT IS NOT NULL AND A.NO_NOTA_DELIVERY <> 0 AND B.FL_PRODUCTION ='N'
				 GROUP BY B.NO_CONT
