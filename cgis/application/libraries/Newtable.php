...
class Newtable {
	...
	function multiple_searchh($show)
	{
		$this->multiple_searchh = $show;
		return;
	}
	...
	function generate($table_data = NULL){
				...
				if($keys = $this->CI->input->post('form')){
					...
					if($this->multiple_search){
						foreach($keys as $a => $b){
							...
							}else if($type=="FILTERIN"){
								if($cari[0]!=""){
									$tercari .= " AND UPPER($field) IN ($txtcari)";
								}
							}else if($type=="OPTIONIS"){
								if($cari[0]!=""){
									$tercari .= " AND $field IS $txtcari";
								}
							}else{
								if($cari[0]!=""){
		...
		if ($this->show_search){
			...
			if($this->multiple_search){
				...
				$out .=	'<BR>';
				$out .=	'<div class="form-group">';
				$out .=	'<div class="col-sm-12" hidden align="center">';
				$out .=	'<button type="reset" class="btn btn-sm btn-danger waves-effect waves-light" onclick=\'form_reset("'.$this->formid.'")\'  ><i class="icon md-refresh"></i> CANCEL</button>&nbsp;';
				$out .= "<button type=\"submit\" onclick=\"newtable_search('".$this->formid."','".$this->divid."','".$this->hal."','".$this->sortby."','".$this->orderby."'); return false;\" class=\"btn btn-sm btn-primary waves-effect waves-light\"><i class='icon md-search'></i> SEARCH</button>";
				$out .=	'	</div>';
				$out .=	'</div>';
			...
			if($this->tipe_proses=="button"){
				...
				foreach ($this->proses as $a => $b){
					if($b[6] == "menu"){
						...
						$out.="<button type='button' class='btn' style='background-color: ".$b[4]."; color: black;'>".$b[5]."</button>";
						$out.="&nbsp;&nbsp;&nbsp;";
					}
					$m++;
				}
		...
		if (count($this->rows) > 0){
			...
			foreach($this->rows as $row){
				...
				if($this->tipe_proses=="button"){
					...
					foreach ($this->proses as $a => $b){
						if($b[6] == "list"){
							$out.= "<button type=\"button\" onclick=\"button_list('".$this->formid."',this.id,this.value)\" id=\"tb_menu".$this->formid.$m."\" formid=\"".$this->formid."\" div=\"".$this->divid."\" met=\"".$b[0]."\" url=\"".$b[1]."\" jml=\"".$b[2]."\" status=\"".$b[3]."\" popup=\"".$b[5]."\" value=\"".$keyz."\" class=\"btn btn-icon btn-primary waves-effect waves-light waves-round\" data-original-title=\"Edit\" data-toggle=\"tooltip\" title=\"".$a."\"><i class=\"".$b[4]."\" aria-hidden=\"true\" div=\"".$this->divid."\"></i></button>";
							$out.="&nbsp;";
						}else if($b[6] == "list2"){
	...
	function generate_custom($table_data = NULL){
				...
				if($keys = $this->CI->input->post('form')){
					...
					if($this->multiple_search){
						foreach($keys as $a => $b){
							...
							}else if($type=="DATERANGE_JI"){
								if(count($cari) > 0){
									$start = $cari[0];
									$end = $cari[1];
									if(($start!="")&&($end!="")){
										$tercari .= " AND (DATE($field) BETWEEN '".$start."' AND '".$end."' OR DATE(az.TG_RESPON) BETWEEN '".$start."' AND '".$end."' OR DATE(az.TGL_DAFTAR_PABEAN) BETWEEN '".$start."' AND '".$end."')";
									}else if($start!=""){
										$tercari .= " AND (DATE($field) >= '".$start."' OR DATE(az.TG_RESPON) >= '".$start."' OR DATE(az.TGL_DAFTAR_PABEAN) >= '".$start."')";
									}else if($end!=""){
										$tercari .= " AND (DATE($field) <= '".$end."' OR DATE(az.TG_RESPON) <= '".$end."' OR DATE(az.TGL_DAFTAR_PABEAN) <= '".$end."')";
									}
								}
							}else if($type=="DATETIMERANGE"){
								if(count($cari) > 0){
							...
							}else if($type=="DOKUMEN_JI"){
								if($cari[0]!=""){
									$tercari .= " AND (UPPER($field) LIKE '%$txtcari%' OR UPPER(az.NO_RESPON) LIKE '%$txtcari%' OR UPPER(az.NO_DAFTAR_PABEAN) LIKE '%$txtcari%')";
								}
							}else if($type=="PEMERIKSA_JI"){
								if($cari[0]!=""){
									$tercari .= " AND (UPPER($field) LIKE '%$txtcari%' OR UPPER(az.namabc) LIKE '%$txtcari%')";
								}
							}else{
								if($cari[0]!=""){
									$tercari .= " AND UPPER($field) LIKE '%$txtcari%'";
		...
		if (count($this->rows) > 0){
			...
			foreach($this->rows as $row){
				...
				$out .= $this->newline;
				if($row['STATUS2'] == "1"){
					$cels = $this->template['cell_val1'];
				}else if($row['STATUS2'] == "2"){
					$cels = $this->template['cell_val2'];
				}else if($row['STATUS2'] == "3"){
					$cels = $this->template['cell_val3'];
				}else if($row['STATUS2'] == "4"){
					$cels = $this->template['cell_val4'];
				}else if($row['STATUS2'] == "5"){
					$cels = $this->template['cell_val5'];
				}else if($row['STATUS2'] == "6"){
					$cels = $this->template['cell_val6'];
				}
				$out .= $cels.$x.'</td>';
				if(in_array($keyz,$tmpchkarray)) $checked = "checked";
				else $checked = "";
	...
	function generate_custom_row($table_data = NULL){
		if ( ! is_null($table_data))
		{
			if (is_object($table_data)){
				$this->_set_from_object($table_data);
	...
	function _default_template(){
		return  array (
						...
						'cell_val1'				=> '<td '.$this->td_click.' class="val1">',
						'cell_val2'				=> '<td '.$this->td_click.' class="val2">',
						'cell_val3'				=> '<td '.$this->td_click.' class="val3">',
						'cell_val4'				=> '<td '.$this->td_click.' class="val4">',
						'cell_val5'				=> '<td '.$this->td_click.' class="val5">',
						'cell_val6'				=> '<td '.$this->td_click.' class="val6">',
						'cell_end'				=> '</td>',
						'row_alt_start' 		=> '<tr>',
						'row_alt_end' 			=> '</tr>',
