...
class Dashboard extends CI_Controller
{
	...
	public function profile($act, $id)
	{
		...
		$data = $this->load->view('content/dashboard/profile', '', true);
	...
	public function jobManager($job, $remark)
	{
		if (!$this->session->userdata('LOGGED')) {
			$this->index();
			return;
		...
		$selectedNew = $this->input->post('vals');
		//print_r($this->input->post('vals'));
		//echo count($selectedNew);
		if ($selected == '') {
			$arrdata['dash'] = $this->m_home->get_data('dashboard', $job, $remark);
			$data = $this->load->view('content/dashboard/dashboardMonitoring', $arrdata, true);
			if ($this->input->post("ajax") || $act == "post") {
				echo $arrdata;
			} else {
				$this->content = $data;
			...
			$countKodeStatus = count($selected);
			$tmpArr = '';
			$job = '';
			$remark = '';
			for ($i = 0; $i < $countKodeStatus; $i++) {
				$tmpArr .= "'" . $selected[$i] . "',";
				$panjangChar = strlen($selected[$i]);
				if ($panjangChar != 3) {
					$remark .= "'" . $selected[$i] . "',";
				} else {
					$job .= "'" . $selected[$i] . "',";
			...
			$resultJob = rtrim($job, ", ");
			$resultRemark = rtrim($remark, ", ");
			$resultArrSelected = rtrim($tmpArr, ", ");
			$arrdata['dash'] = $this->m_home->get_data('dashboard', $resultJob, $resultRemark);
			//print_r($arrdata);
			$data = $this->load->view('content/dashboard/dashboardMonitoring', $arrdata, true);
			if ($this->input->post("ajax") || $act == "post") {
				echo $arrdata;
	...
	public function filter()
	{
		...
		$this->load->model("m_home");
		$arrdata = $this->m_home->get_data('dashboardNew', $resultJob, $resultRemark);
		print_r($arrdata); //die();
		//$this->dashMonitoring($resultJob,$resultRemark);
	}
	...
	public function filterOld()
	{
		...
		//	print_r($this->input->post('vals'));die();
		$selected = $this->input->post('selected');
		print_r($this->input->post('vals'));
		$countKodeStatus = count($selected);
		$tmpArr = '';
	...
	{ {
			...
			$this->load->model("m_home");
			$arrdata = $this->m_home->dashboardautogate();
