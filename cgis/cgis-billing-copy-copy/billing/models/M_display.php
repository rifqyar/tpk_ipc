...
class M_display extends CI_Model {
	...
	function get_data($act, $id){
        	...
        	if($act == 'custom'){
        		...
        		$SQL = "SELECT C.ID AS 'ID',A.NO_SPK AS 'NO_SPK', CONCAT(B.NO_CONT,'<BR>',B.UKR_CONT) AS 'KONTAINER',C.NO_DOK,G.NAMA, DATE_FORMAT(C.WK_ACTIVE, '%d-%m-%Y %h:%m:%s') AS 'WK_ACTIVE', 
					CONCAT(B.LOKASI,'0',B.TIER) AS 'LOKASI', C.JNS_KEGIATAN,A.CONSIGNEE AS 'NAMA_CUSTOMER', 
					CASE WHEN B.STATUS_CONT = '460' AND F.START_INSP IS NULL AND F.FINISH_INSP IS NULL AND C.JNS_KEGIATAN ='1' THEN 'SIAP PERIKSA' 
					WHEN B.STATUS_CONT ='460' AND C.JNS_KEGIATAN='2' THEN 'SIAP PERIKSA'
					WHEN B.STATUS_CONT IN ('500','540','520') THEN 'SELESAI PERIKSA' 
					WHEN B.STATUS_CONT = '460' AND F.START_INSP IS NOT NULL AND F.FINISH_INSP IS NULL THEN 'SEDANG PERIKSA' ELSE 'ANTRIAN PERIKSA' END AS 'WSTATUS',C.RESPON AS 'RESPON',DATE_FORMAT(C.WK_RESPON, '%d-%m-%Y %h:%m:%s') AS 'WK_RESPON'
					FROM t_spk A
					...
					WHERE B.STATUS_CONT IN('460','500','510','530','540','520') AND C.FL_ACTIVE = 'Y' AND G.ID != 83
					GROUP BY B.NO_CONT
					ORDER BY 
	...
	public function getresponcustoms($act, $id){
		if($this->session->userdata('KD_GROUP')=="BC"){
			...
			$SQL = "SELECT C.ID AS 'ID',A.NO_SPK AS 'NO_SPK',CONCAT(B.NO_CONT,'<BR>',B.UKR_CONT) AS 'KONTAINER',CONCAT('NO :&nbsp',C.NO_DOK,'<BR>JNS :&nbsp',G.NAMA) AS 'DOKUMEN',CONCAT(B.LOKASI,'0',B.TIER) AS 'LOKASI', CASE WHEN C.JNS_KEGIATAN = '1' THEN 'BEHANDLE 1' WHEN C.JNS_KEGIATAN = '2' THEN 'BEHANDLE 2' END AS 'KETERANGAN',A.CONSIGNEE AS 'NAMA CUSTOMER', 
					
					CASE WHEN B.STATUS_CONT = '460' AND F.START_INSP IS NULL AND F.FINISH_INSP IS NULL  AND C.JNS_KEGIATAN ='1' THEN '<span class=\"label label-warning\">SIAP PERIKSA</span>' 
					WHEN B.STATUS_CONT ='460' AND C.JNS_KEGIATAN='2' THEN '<span class=\"label label-warning\">SIAP PERIKSA</span>' 
			...
			$SQL = "SELECT C.ID AS 'ID',A.NO_SPK AS 'NO_SPK',CONCAT(B.NO_CONT,'<BR>',B.UKR_CONT) AS 'KONTAINER',CONCAT('NO :&nbsp',C.NO_DOK,'<BR>JNS :&nbsp',G.NAMA) AS 'DOKUMEN',CONCAT(B.LOKASI,'0',B.TIER) AS 'LOKASI', CASE WHEN C.JNS_KEGIATAN = '1' THEN 'BEHANDLE 1' WHEN C.JNS_KEGIATAN = '2' THEN 'BEHANDLE 2' END AS 'KETERANGAN',A.CONSIGNEE AS 'NAMA CUSTOMER', 
					...
					WHEN B.STATUS_CONT = '460' AND F.START_INSP IS NOT NULL AND F.FINISH_INSP IS NULL THEN '<span class=\"label label-primary\">SEDANG PERIKSA</span>' 
					ELSE '<span class=\"label label-danger\">ANTRIAN PERIKSA</span>' END AS 'STATUS',
					CASE WHEN C.RESPON = 'PKB PRIORITAS' THEN '<span style=\"color:green;font-weight:bold\">PKB PRIORITAS</span>' WHEN C.RESPON = 'PKB LR' THEN '<span style=\"color:green;font-weight:bold\">PKB LONGROOM</span>' WHEN C.RESPON = 'PKB YARD' THEN '<span style=\"color:green;font-weight:bold\">PKB YARD</span>' WHEN C.RESPON = 'PKB YARD N' THEN '<span style=\"color:green;font-weight:bold\">PKB YARD N</span>' ELSE '<span style=\"color:red;font-weight:bold\">NO RESPON</span>' END AS 'RESPON', C.WK_RESPON AS 'WK_RESPON' 
					FROM t_spk A
					LEFT JOIN t_spk_cont B ON A.ID = B.ID
					LEFT JOIN t_gatepass C ON B.NO_CONT = C.NO_CONT AND C.`STATUS` = 'WAITING'
					INNER JOIN t_request D ON C.NO_DOK = D.NO_DOK
			...
			$this->newtable->breadcrumb('Customer', 'javascript:void(0)','');
			$check = (grant()=="W")?true:false;
			$addsql = " B.STATUS_CONT IN('460','500','510','530','540','520') AND C.FL_ACTIVE = 'Y'";
			$SQL = "SELECT C.ID AS 'ID',A.NO_SPK AS 'NO_SPK',CONCAT(B.NO_CONT,'<BR>',B.UKR_CONT) AS 'KONTAINER',CONCAT('NO :&nbsp',C.NO_DOK,'<BR>JNS :&nbsp',G.NAMA) AS 'DOKUMEN',CONCAT(B.LOKASI,'0',B.TIER) AS 'LOKASI',	CASE WHEN C.JNS_KEGIATAN = '1' THEN 'BEHANDLE 1' WHEN C.JNS_KEGIATAN = '2' THEN 'BEHANDLE 2' END AS 'KETERANGAN',A.CONSIGNEE AS 'NAMA CUSTOMER', 
