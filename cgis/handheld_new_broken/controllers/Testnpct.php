...
class Testnpct extends CI_Controller
{
    public $content;
    ...
    public function RequestGatepassSPPMP(){
        ...
        A.ID, UPPER(B.KODE_NPCT1) AS JNS_DOK_DESC, A.NO_BL_AWB AS NO_BL_AWB
        FROM t_request A
        LEFT JOIN reff_kode_dok_bc B ON A.JNS_DOK = B.ID
        WHERE A.KD_REQ = 'QUEUED'AND A.ID ='86698' -- AND A.NO_DOK='CN26/2018-3038'
        ORDER BY A.ID DESC
        LIMIT 10";
        // echo $SQL;die();
...
CURLOPT_SSL_VERIFYHOST => false,
    ...
    public function tracking(){
        ...
        WHERE fl_track = 'N' and no_cont ='MEDU2098922'");
...
CURLOPT_SSL_VERIFYHOST => false,
    ...
    public function getInquiry(){
        ...
        $SQL = $this->db->query("SELECT DISTINCT B.ID, A.JNS_DOK, B.NO_CONT, B.REF_NUMBER FROM t_request A INNER JOIN t_request_cont B ON A.ID = B.ID WHERE A.ID ='86698' AND B.KD_STATUS='APPROVED' AND B.REF_NUMBER IS NOT NULL")->row_array();
...
CURLOPT_SSL_VERIFYHOST => false,
    ...
    public function testparse(){
            ...
            <desc>Successfully request data</desc>
            <data>
                <loop>
                    <vessel_id>BORES</vessel_id>
                    <vessel_name>BOMAR RESILIENT</vessel_name>
                    <call_sign>D5UN9</call_sign>
                    <voyage_in>FC123A</voyage_in>
                    <voyage_out>FC123A</voyage_out>
                    <cont_no>MEDU2534517</cont_no>
                    <cont_size>20</cont_size>
                    <full_empty>F</full_empty>
                    <isocode>2210</isocode>
                    <reefer>N</reefer>
                    <imdg>N</imdg>
                    <oog>N</oog>
                    <in_time>20210902101919</in_time>
                    <hold>N</hold>
                </loop>
                <loop>
                    <vessel_id>BORES</vessel_id>
    ...
    public function testparseinquiry(){
                $xml = '<response>
                <data>
                    <loop>
                        ...
                        <full_empty>F</full_empty>
                        <in_out>OUT</in_out>
                        <tar>RDC537G/NPCT1</tar>
                        <vessel_name>ALS CERES</vessel_name>
                        <voyage_in>IT UAT1</voyage_in>
                        <voyage_out>IT UAT1</voyage_out>
                        <paidthrough>20210825235500</paidthrough>
                        <pod>IDJKT</pod>
                        <spod>AUSYD</spod>
                        <customer_name>PT. PERUSAHAAN PERDAGANGAN INDONESIA (PERSERO)</customer_name>
                        <cust_type>SPPMP</cust_type>
                        <cust_no>2021.2.0300.0.K02.I.017633</cust_no>
                        <cust_date>20210819</cust_date>
                        <imdg>N</imdg>
                        <imdg_value></imdg_value>
            ...
            // var_dump($xml);die();
            $loop = $xml->data->loop;
            $tgldok = $xml->data->loop->paidthrough;
            $countloop = count($loop);
            $tgldok = Date('Y-m-d H:m:s.0', strtotime(str_replace("","-",($tgldok))));
            var_dump($tgldok);die();
            $ID='23900';
    ...
    public function testparsegatepass(){
            $xml = '<response>
                <gatepass>
                    <reff_number>CGO0093629</reff_number>
                    <status>APPROVED</status>
                    <cont_no>EISU8221414</cont_no>
                </gatepass>
            </response>';
    ...
    public function testinquiryGatepass(){
		...
		// $REF_NUMBER = $SQL['REF_NUMBER'];
        $REF_NUMBER = 'CGO0091915';

        $addXML ='<request><reff_number>'.$REF_NUMBER.'</reff_number></request>';
...
CURLOPT_SSL_VERIFYHOST => false,
            ...
            $VESSEL_NAME = $xml->containers->vessel_name;
            $CALL_SIGN = $xml->containers->call_sign;
            $VOYAGE_IN = $xml->containers->voyage_in;
            $VOYAGE_OUT = $xml->containers->voyage_out;
            $CONT_NO = $xml->containers->cont_no;
            $SIZE = $xml->containers->cont_size;
            $JENIS = $xml->containers->full_empty;
            $slice = substr($SIZE, 0, 2);
            $ISOCODE = $xml->containers->isocode;
            $REEFER = $xml->containers->reefer;
            $REQ_TEMP = $xml->containers->reefer_req_temp;
            $ACT_TEMP = $xml->containers->reefer_act_temp;
            $PLUG = $xml->containers->reefer_plug_in;
            $UNPLUG = $xml->containers->reefer_plug_out;
            $IMDG = $xml->containers->imdg;
            $OOG = $xml->containers->oog;
            $DISCHARGE = $xml->containers->discharge;
            $tgl_bongkar = date_format(new DateTime($DISCHARGE), 'Y-m-d H:i:s');
            $GATEOUT = $xml->containers->gateout;
            $REFERENCE_NO = $xml->containers->reference_no;
            $HOLD = $xml->containers->hold;
            $ON_YARD = $xml->containers->on_yard;
