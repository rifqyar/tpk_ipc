...
class Status extends CI_Controller{
	...
	public function kontainer($act,$id){
		if (!$this->session->userdata('LOGGED')){
			...
			if($act=="add"){
				...
				$arrdata = $this->m_status->kontainer($act, $id);
	...
	function status_container(){
		...
		$arrayData = $this->m_dashboard->status_container();
		$arrdata = array('arrdata' => $arrayData);
		$this->load->view('dashboard/status_container', $arrdata);
	}
	
	function status_container_update(){
		$this->load->model('m_dashboard');
		$this->m_dashboard->status_container_update();
	}
	
	function status_container_detail(){
		$arrdata = array();
		$this->load->model('m_dashboard');
		$arrayData = $this->m_dashboard->status_container_detail();
		$arrdata = array('arrdata' => $arrayData);
		$this->load->view('dashboard/status_container_detail', $arrdata);
	}
	
	function status_container_detail_update(){
		$this->load->model('m_dashboard');
		$this->m_dashboard->status_container_detail_update();
	}
}
