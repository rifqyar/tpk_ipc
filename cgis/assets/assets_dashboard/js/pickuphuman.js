...
$(document).on('click','a#send',function () {
    ...
    if (plat != 'NONE') {
        ...
        $.ajax({
            type: "POST",
            url: '/tpk_ipc/cgis/application.php/PortalDashboard/ubahunplug',
            data: {container:cont,no_plat:plat},
            dataType: 'json',
            success: function(response){
                //console.log(response);
                if (response.status == 'success') {
                    $('.'+cont).replaceWith(`<a class='btn btn-success ${cont}' style='display: inherit;'>S</a>`);
                    swal("Info", "Terikirim", "success")
                }else{
...
$(document).on('click','a#pickupajax',function () {
    ...
    $.ajax({
        type: "POST",
        url: '/tpk_ipc/cgis-dev/application.php/PortalDashboard/pickuppercontainer',
