	...
	function popup_search($act,$id,$popup,$ajax){
		...
		if($arract[0]=="discharge_kontainer"){
			...
			$SQL = "SELECT ID, NAMA, KD_PERMIT
					FROM reff_kode_dok_bc WHERE ID != '83' AND 1=1".$addsql;
			...
			$SQL = "SELECT X.JNS_DOK, X.NO_DOK, X.TGL_DOK
					FROM
					(SELECT B.NAMA AS 'JNS_DOK', A.NO_DAFTAR_PABEAN AS 'NO_DOK', A.TGL_DAFTAR_PABEAN AS 'TGL_DOK'
					FROM t_permit_hdr A
					INNER JOIN reff_kode_dok_bc B ON A.KD_DOK_INOUT = B.ID
					WHERE A.KD_DOK_INOUT IN('19','81','82','83')
					UNION ALL
