...
class M_operation extends CI_Model{
				...
				if($SQL_SPK_CONT[0]['STATUS_CONT'] == '100'){
					...
					for ($c=0; $c < count($SQL_SPK_CONT); $c++) {
						...
						$operation = array(
							...
							'JNS_DOK' => $SQL_SPK_CONT['JNS_DOK'],
							'NO_DOK' => $SQL_SPK_CONT['NO-DOK'],
	...
	public function set_inspection(){
		...
		$SQL_SPK	= $this->db->query("SELECT A.*, B.* FROM t_spk_cont A INNER JOIN t_spk B ON A.ID = B.ID WHERE A.NO_CONT ='".$noCont."' AND A.STATUS_CONT ='200' AND B.KD_STATUS='200' ORDER BY A.ID DESC LIMIT 1")->row_array();
			...
			if(substr($lokasi,0,2)=="1B" || substr($lokasi,0,2)=="1b" || substr($lokasi,0,2)=="YB"){
				if($SQL_SPK_LOK['LOKASI'] != $lokasi){
					if ($SQL_DENAH['DENAH'] == $lokasi) {
						...
						/* CEK GATEPASS */
						$SQL_GATEPASS = $this->db->query("SELECT * FROM t_gatepass WHERE NO_CONT = '".$noCont."' AND NO_DOK = '".$SQL_SPK['NO_DOK']."' AND STATUS = 'WAITING' AND JNS_KEGIATAN ='1' ORDER BY ID DESC LIMIT 1")->row_array();
						...
						$this->db->update('t_job_slip', array('NO_DOK' => $SQL_SPK['NO_DOK'], 'NO_GATEPASS' => $SQL_GATEPASS['ID'], 'TGL_GATEPASS' => $SQL_GATEPASS['WK_REK'], 'STATUS' => 'DONE',  'KD_STATUS' => '50', 'WK_STATUS' => date('Y-m-d H:i:s'), 'OPERATOR' => $this->session->userdata('USERLOGIN'), 'STATUS_JOB' => 'Y'));
						...
						if(count($SQL_GATEPASS) > 0){
							$arrdata = array(
								...
								'NO_GATEPASS'	=> $SQL_GATEPASS['ID'],
								'LOKASI_AWAL'	=> $nmBlok,
								'JNS_JOB_SLIP'	=> 'MARSHALLING',
								'JENIS'			=> 'BEHANDLE 1',
								'TIER_AWAL'		=> $nmTier,
								'TGL_GATEPASS'	=> $SQL_GATEPASS['WK_REK'],
								'KD_STATUS'		=> '20',
								'STATUS'		=> 'WAITING',
	...
	public function ceklokasiterpakai3($NMBLOK, $TIER){
	...
	public function set_pemeriksaan(){
		...
		$SQL_SPK_CONT    = $this->db->query("SELECT A.*, B.* FROM t_spk_cont A INNER JOIN t_spk B ON A.ID = B.ID WHERE A.NO_CONT ='".$NO_CONT."' AND A.STATUS_CONT = '460' ORDER BY A.ID DESC DESC")->row_array();
	...
	public function ambilmarshcic($keyword){
		$SQL = "SELECT DISTINCT A.ID_JOB_SLIP,A.NO_CONT,C.UKR_CONT,A.LOKASI_AWAL,A.LOKASI_AKHIR,A.TIER_AWAL,A.TIER_AKHIR,A.JENIS,C.RESPON
			FROM t_job_slip A
	...
	public function datajumlahmarshallingcic(){
		$queryspk=$this->db->query("SELECT DISTINCT A.ID_JOB_SLIP,A.NO_CONT,A.LOKASI_AWAL,A.LOKASI_AKHIR
			...
			INNER JOIN t_gatepass C ON A.NO_GATEPASS = C.ID
			WHERE A.STATUS = 'WAITING' AND A.KD_STATUS='20' AND A.LOKASI_AKHIR LIKE 'CIC%'
			AND C.FL_ACTIVE = 'Y' AND A.JENIS IN ('BEHANDLE 1','BEHANDLE 2') GROUP BY A.NO_CONT
			ORDER BY A.ID_JOB_SLIP DESC");
		return $queryspk->result_array();
	}
	...
	public function search($keyword){
			$queryspk=$this->db->query("SELECT ID, NO_CONT, UKR_CONT, 
										...
										FROM t_spk_cont
										WHERE NO_CONT LIKE '%$keyword%' AND STATUS_CONT IN (200) GROUP BY NO_CONT ORDER BY ID DESC");
			return $queryspk->row_array();
    }
    ...
    public function searchreal($keyword){
	        $queryspk=$this->db->query("select * FROM t_spk_cont WHERE NO_CONT LIKE '%$keyword%' AND STATUS_CONT IN('460') ORDER BY ID DESC LIMIT 1");
			return $queryspk->result_array();
    }
	...
	public function set_copy(){
		...
		$LOKASI_LAMA = $this->input->post('nolok');
		$STATUS_BLOK = $this->input->post('mySelect');
		$LOKASI_BARU = $this->input->post('lokbar');

		$SQL_SPK	 = $this->db->query("SELECT A.*, B.*, C.* FROM t_spk_cont A INNER JOIN t_spk B ON A.ID = B.ID INNER JOIN reff_status_spk C ON A.STATUS_CONT = C.ID WHERE A.NO_CONT ='".$NO_CONT."' AND C.KETERANGAN='".$STATUS_CONT."' ORDER BY A.ID DESC LIMIT 1")->result_array();

		if($SQL_SPK[0]['STATUS_CONT'] == '460'){
	...
	public function searchop($NO_CONT){
		$queryspk=$this->db->query("SELECT * FROM t_spk_cont WHERE STATUS_CONT NOT IN('000','100','200') AND NO_CONT LIKE '%$NO_CONT%' ORDER BY ID DESC LIMIT 1");
		return $queryspk->result_array();
	}
			
	public function searchop2($NO_CONT){
		$queryspk=$this->db->query("SELECT A.NO_CONT, A.UKR_CONT, A.LOKASI,A.TIER, B.NO_DOK, C.KETERANGAN, D.NAMA FROM t_spk_cont A INNER JOIN t_spk B ON A.ID = B.ID INNER JOIN reff_status_spk C ON A.STATUS_CONT = C.ID INNER JOIN reff_kode_dok_bc D ON B.JNS_DOK = D.ID WHERE A.STATUS_CONT NOT IN('000','100','200') AND A.NO_CONT = '$NO_CONT' ORDER BY A.ID DESC LIMIT 1");
		return $queryspk->result_array();
	}
