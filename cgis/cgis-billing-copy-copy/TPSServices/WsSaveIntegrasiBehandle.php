...
if (!$CheckFile) {
    ...
    if (date('H') >= $jam_awal && date('H') <= $jam_akhir) {
        $SQL = "SELECT DISTINCT A.ID AS ID, A.ID_REQ AS NO_ORDER, A.NO_NOTA_BEHANDLE AS NO_INVOICE, '' AS EX_NOTA, C.NAMA_BANK AS BANK,
                ...
                WHERE A.NO_NOTA_BEHANDLE IS NOT NULL 
                    -- AND YEAR(A.TGL_NOTA) = '2018' 
                    AND A.NO_NOTA_BEHANDLE = '030.801.18-65.012079'
        ...
        if ($Query->size() > 0) {
            ...
            while ($Query->next()) {
                ...
                $message .= '<header_sub_context>PTKM05</header_sub_context>'; //DARI MAS PANDIT
                $message .= '<organization_id>1827</organization_id>';
                $message .= '<transaction_date>' . $data['TGL_TERIMA'] . '</transaction_date>';
                $message .= '<transaction_type>PELAYANAN JASA BEHANDLE</transaction_type>';
...
//                echo "</pre>";
                ...
                if ($length > 1) {
                    for ($c = 0; $c < $length; $c++) {
                        ...
                        $SQL = "UPDATE req_behandle_hdr SET FL_SEND_TRANS_SIMKEU = '" . $status . "', 
                                                            MESSAGE_SEND_TRANS_SIMKEU = '" . $message . "', 
                                                            TGL_SEND_TRANS_SIMKEU = NOW()
                                WHERE NO_NOTA_BEHANDLE = '" . $transaction_number . "'";
                        echo $SQL . "<br>";
