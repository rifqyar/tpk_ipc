...
class ServiceBC extends CI_Controller
{
    ...
    public function sendcodeco(){
        $q = $this->db->query("SELECT ts.NO_SPK, tc.NM_ANGKUT, tc.NO_VOY_FLIGHT, tc.CALL_SIGN, DATE_FORMAT(tc.TGL_TIBA, '%Y%m%d') as 'TGL_TIBA', tr.REF_NUMBER,
            ...
            where tg.WK_REK > '2022-09-24' and tg.JNS_KEGIATAN = '3' and tod.FL_SEND_BC = 'N' and tod.WK_GATEOUT is not null limit 20");
        foreach ($q->result() as $key => $value1) {
            ...
            <REF_NUMBER>NCT1'.$value1->REF_NUMBER.'9</REF_NUMBER>
            </HEADER>
            <DETIL>
            <CONT>
            ...
            <!--Optional:-->
            <ser:fStream><![CDATA['.$cdata.']]></ser:fStream>
            <!--Optional:-->
            ...
            $this->db->query("INSERT INTO log_send_bc (NO_SPK, NO_CONTAINER, RESPONDATA, XML, WK_SEND)  VALUES ('$value1->NO_SPK', '$value1->NO_CONT', '$responnyagan', '$cdata', '$datenow')");
    ...
    public function sendcodecomanual(){
        $q = $this->db->query("SELECT ts.NO_SPK, tc.NM_ANGKUT, tc.NO_VOY_FLIGHT, tc.CALL_SIGN, DATE_FORMAT(tc.TGL_TIBA, '%Y%m%d') as 'TGL_TIBA', tr.REF_NUMBER,
            ...
            join t_op_delivery tod on tg.NO_SPK = tod.NO_SPK and tg.NO_CONT = tod.NO_CONT
            join t_permit_hdr tph on tg.NO_DOK = tph.NO_DOK_INOUT
            where tg.WK_REK > '2022-09-24' and tg.JNS_KEGIATAN = '3' and tod.NO_CONT = 'TCNU5951292'");
        foreach ($q->result() as $key => $value1) {
            $cdata = '<?xml version="1.0" encoding="utf-8"?>
            <DOCUMENT
            xmlns="cococont.xsd">
    ...
    public function getdocsppb(){
        ...
        curl_close($curl);
        $xml1 = str_replace('&lt;',"<",$response);
        $xml2 = str_replace('&gt;',">",$xml1);
        $dirtyxml = str_ireplace('<?xml version="1.0" encoding="utf-8"?>
<soap:Envelope
    xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/"
    xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    xmlns:xsd="http://www.w3.org/2001/XMLSchema">
    <soap:Body>','', $xml2);
        $clean_xml = str_ireplace('soap:Body','', $dirtyxml);
        $xml = simplexml_load_string($clean_xml);
        echo $clean_xml;
    }
}