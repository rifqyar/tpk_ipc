...
class Apibos_dev extends CI_Controller {
    ...
    public function updateGatepassDelivery($cek_nodok,$id_req){
		...
		if ($cekTotalGatePass) {
			...
			for ($i=0; $i <= $totalCont ; $i++) { 
                ...
                //kayaknya ini yang bikin gatepass otomatis keupdate pas user belum bayar
	...
	public function billingbhd(){
		...
		try {
			...
			$id_bhd_billing = $this->input->post('ID_BHD_BILLING');
			...
			WHERE A.NO_RESPON = '$nodok' and A.TG_RESPON = '$tgdok' AND C.JNS_KEGIATAN !='' AND C.FL_BIL IS NULL and C.NO_CONT in ($arrnocont2)
			GROUP BY C.NO_CONT, C.JNS_KEGIATAN");

            if ($cekdetail->num_rows() == 0) {
                $cekdetail = $this->db->query("SELECT DISTINCT A.ID, C.JNS_DOK, A.NO_DOK_INOUT , A.TGL_DOK_INOUT , C.NO_CONT, C.UKR_CONT, 
                CASE WHEN C.JNS_KEGIATAN = '1' AND H.LNSW_NOAJU IS null THEN 'BEHANDLE 1' WHEN C.JNS_KEGIATAN ='2' AND H.LNSW_NOAJU IS null THEN 'BEHANDLE 2' WHEN H.LNSW_NOAJU IS not NULL then 'BEHANDLE JOIN' ELSE '' END AS JNS_KEGIATAN
                FROM t_permit_hdr A
                INNER JOIN t_permit_cont B ON A.ID = B.ID
                INNER JOIN t_gatepass C ON A.NO_DOK_INOUT = C.NO_DOK AND B.NO_CONT = C.NO_CONT AND C.STATUS ='DONE'
                INNER JOIN t_op_inspection D ON B.NO_CONT = D.NO_CONT AND C.NO_CONT = D.NO_CONT AND D.STATUS = 'DONE'
                LEFT JOIN v_ppk_permit_join H ON C.NO_DOK = H.NO_DAFTAR_PABEAN AND C.NO_CONT = H.NO_CONT
                WHERE A.NO_DOK_INOUT = '$nodok' and A.TGL_DOK_INOUT = '$tgdok' AND C.JNS_KEGIATAN !='' AND C.FL_BIL IS NULL and C.NO_CONT in ($arrnocont2)
                GROUP BY C.NO_CONT, C.JNS_KEGIATAN");
            }
            ...
            foreach($this->input->post() as $a => $b) {
                if ($a === 'TOKEN') {
                    $DATA[$a] = $b; // biarkan token tetap
                    continue;
                }
                if ($b === "") continue; // abaikan yang kosong
                ...
                $DATA_BHD = array();
                $DATA_BHD['ID_REQ']     = $REQ;
                $DATA_BHD['JNS_DOK']    = isset($DATA['JNS_DOK']) && $DATA['JNS_DOK'] !== '' ? $DATA['JNS_DOK'] : NULL;
                $DATA_BHD['NO_DOK']     = isset($DATA['NO_DOK']) && $DATA['NO_DOK'] !== '' ? $DATA['NO_DOK'] : NULL;
                $DATA_BHD['TGL_DOK']    = isset($DATA['TGL_DOK']) && $DATA['TGL_DOK'] !== '' ? $DATA['TGL_DOK'] : NULL;
                $DATA_BHD['NM_KAPAL']   = isset($nmkapalup) && $nmkapalup !== '' ? $nmkapalup : NULL;
                $DATA_BHD['NO_VOY']     = isset($voyup) && $voyup !== '' ? $voyup : NULL;
                $DATA_BHD['NO_DO']      = isset($DATA['NO_DO']) && $DATA['NO_DO'] !== '' ? $DATA['NO_DO'] : NULL;
                $DATA_BHD['NO_BL']      = isset($DATA['NO_BL']) && $DATA['NO_BL'] !== '' ? $DATA['NO_BL'] : NULL;
                $DATA_BHD['TGL_REQ']    = date('Y-m-d H:i:s');
                $DATA_BHD['NAMA_CUST']  = isset($DATA['CUSTOMER']) && $DATA['CUSTOMER'] !== '' ? $DATA['CUSTOMER'] : NULL;
                $DATA_BHD['OPERATOR']   = 'online';
                $DATA_BHD['NPWP']       = isset($DATA['NPWP']) && $DATA['NPWP'] !== '' ? $DATA['NPWP'] : NULL;
				...
				for($x=0; $x<$jumlah_dipilih; $x++){
					...
					if ($kapalsandar == 1) {
					    //update 11 April, update dari PAKET BEHANDLE menjadi pemeriksaan fisik
						$SQL = $this->db->query("SELECT * FROM m_tarif2 WHERE SIZE = '$SIZE' AND JENIS_BIAYA = 'PEMERIKSAAN FISIK' AND KET LIKE '$KEGIATAN'")->row();	
					}else{
						$SQL = $this->db->query("SELECT * FROM m_tarif WHERE SIZE = '$SIZE' AND JENIS_BIAYA = 'PAKET BEHANDLE' AND KET LIKE '$KEGIATAN'")->row();
					}

                    // biaya hico scan
				    // //update 11 April, update dari HICO SCAN menjadi paket behandle
                    if($SCAN){
                        $SQLSCAN = $this->db->query("SELECT * FROM m_tarif2 WHERE SIZE = '$SIZE' AND JENIS_BIAYA = 'PAKET BEHANDLE'")->row();
					$DETAIL['ID_REQ'] = $REQ;
					$DETAIL['NO_CONT'] = $DT['NO_CONT'][$x];
					$DETAIL['UK_CONT'] = $DT['UKR_CONT'][$x];
					$DETAIL['JNS_KEGIATAN'] = $SQLSCAN->JENIS_BIAYA; 
					$DETAIL['TOTAL'] = $SQLSCAN->TARIF; 
					$DETAIL['TARIF_ID'] = $SQLSCAN->TARIF_ID; 
	...
	public function billingdelivery()
    {
        ...
        try {
			...
			$id_del_billing = $this->input->post('ID_BHD_BILLING');
			$id_dokumen = $this->input->post('ID_DOKUMEN');
            $nodok = $this->input->post('NO_DOK');
			...
			$bayar = '000';
            $filedok = $this->input->post('FILE_DOK');
            ...
            // script ini bikin gak bisa buat ulang billing delivery
                ...
                // }
            // 29 November 2024
            // hotfix cek kalo dokumen yang direquest hanya yang dari tps online bukan dokumen manual
            ...
            if ($code === '00') {
                $query = $this->db->query("SELECT * from t_permit_hdr where NO_DOK_INOUT = '$NO_SPPB' and TGL_DOK_INOUT = '$TGL_SPPB'");
                    ...
                    if ($count === 0){
                        $this->db->query("INSERT INTO t_permit_hdr (CAR, NO_DOK_INOUT, KD_DOK_INOUT, TGL_DOK_INOUT, KD_KANTOR, NO_DAFTAR_PABEAN, TGL_DAFTAR_PABEAN, ID_CONSIGNEE, CONSIGNEE, ALAMAT_CONSIGNEE, NPWP_PPJK, NAMA_PPJK, ALAMAT_PPJK, NM_ANGKUT, NO_VOY_FLIGHT, BRUTO, NETTO, KD_GUDANG, STATUS_JALUR, JML_CONT, NO_BC11, TGL_BC11, NO_POS_BC11, NO_BL_AWB, TGL_BL_AWB, NO_MASTER_BL_AWB, TGL_MASTER_BL_AWB, OPERATOR, TGL_STATUS)
                        VALUES ('$CAR', '$NO_SPPB', '1','$TGL_SPPB', '$KD_KPBC', '$NO_PIB', '$TGL_PIB', '$NPWP_IMP', '$NAMA_IMP', '$ALAMAT_IMP', '$NPWP_PPJK', '$NAMA_PPJK', '$ALAMAT_PPJK', '$NM_ANGKUT', '$NO_VOY_FLIGHT', '$BRUTO', '$NETTO', '$GUDANG', '$STATUS_JALUR', '$JML_CONT', '$NO_BC11', '$TGL_BC11', '$NO_POS_BC11', '$NO_BL_AWB', '$TG_BL_AWB', '$NO_MASTER_BL_AWB', '$TG_MASTER_BL_AWB', 'AUTO ONDEMAND SPPB VIA OSBOS', '$datenow')");
                ...
                return $this->httpres(200, 'error', '', 'Dokumen Tidak Ditemukan atau Dokumen Sppb belum tervalidasi dengan tps online, Silahkan Hubungi Bea Cukai');
            }
            $this->db->query("INSERT INTO `tpk_ipc`.`solver_req_dokumen_log` (`url`, `tipe`, `no_dok`, `tgl_dok`, `response_log`) VALUES ('http://10.244.20.14:83/api/v1/get-customs-ondemand', 'sppbbyuser', '$NO_SPPB', '$TGL_SPPB', '$response')");

            $this->db->query("INSERT INTO `tpk_ipc`.`log_services` (`METHOD`, `XML_REQUEST`, `XML_RESPONSE`, `WK_REKAM`, `FL_NPCT1`, `FL_SENT_RIZKI`) VALUES ('GET DOKUMEN SPPB VIA OSBOS', '$addXML', '$response', '$datenow', 'N', 'N')");
            } 
            
            // CEK ULANG KARENA KADANG TPS ONLINE RETURN KOSONGAN
            $receksppb = $this->db->query("SELECT A.ID,C.JENIS from t_permit_hdr A join t_permit_cont B on A.ID = B.ID
            join reff_kode_dok_bc C on A.KD_DOK_INOUT = C.ID
            where C.JENIS = 'RELEASE' and A.FL_MANUAL = 'N' and A.NO_DOK_INOUT = '$no_dok' and B.NO_CONT in ($arrnocont2)")->result();
            if (!$receksppb) {
                return $this->httpres(200, 'error', '', 'Dokumen Tidak Ditemukan atau Dokumen Sppb Belum terbit, Silahkan Hubungi Bea Cukai');
            }
		...
		where A.KD_STATUS_BIL is null and B.NO_DOK_INOUT = '$no_dok' and B.FL_MANUAL = 'N' and A.NO_CONT in ($arrnocont2) and F.STATUS_CONT != '900'
		group by C.NO_CONT,E.NO_DOK")->result();
            ...
            if ($custom_refer == 'on' && $custom_unplug != '') {
                ...
                $PaidThruReferc = date('Y-m-d', strtotime($this->input->post('PAIDTHRU')));
            }
            $error = 0;
            if ($act == "save") {
                ...
                $skip_keys = array('TOKEN', 'arrnocont', 'unplugrefer1'); // field yang bukan bagian dari DATA

                foreach ($input as $key => $val) {
                    if (in_array($key, $skip_keys)) {
                        continue; // lewati field di luar DATA
                    }
                    ...
                    $DATA[$key] = strtoupper(trim($val)); // simpan field setelah dibersihkan
                }
                ...
                for ($x = 0; $x < $JML_CONT; $x++) {
                    ...
                    // return $this->httpres(200, 'error', '', 'CEK DATA INI: ' . json_encode($DTL));
					...
					WHERE B.NO_CONT='$NO_CONT' AND A.NO_DOK_INOUT='$NO_DOK' AND C.PLUG_TERMINAL IS NOT NULL AND d.waktu_end IS NOT NULL ORDER BY c.PLUG_TERMINAL desc,D.WAKTU_END desc LIMIT 1")->row();
                        ...
                        if ($cek == null) {
                            ...
                            return $this->httpres(200, 'error', '', 'Tgl Stacking Tidak Ada untuk kontainer '.$NO_CONT.' ');
                            die();
    ...
    public function billingextdelivery()
    {
        ...
        try {
            $DATAR = $this->input->post('DATA');
			$id_ext_billing = $this->input->post('ID_EXT_BILLING');
			$id_dokumen =$this->input->post('ID_DOKUMEN');
            $nodok = $this->input->post('NO_DOK');
            $no_dok = $this->input->post('NO_DOK');
			$tgdok = $this->input->post('TGL_DOK');
	...
	public function syncdok_dll()
	{
		...
		try {
			$key = "bos_mti";
			$decoded = JWT::decode($token, $key, array('HS256'));
            $q = $this->db->query("select distinct a.NO_DOK ,a.TGL_DOK ,e.NAMA, b.NO_CONT ,b.UKR_CONT,b.VESSEL,b.VOY_IN, b.DISCHARGE ,b.STATUS_BILLING ,c.KD_STATUS ,d.STATUS_CONT,
            CASE WHEN x.SAP_TGL_PELUNASAN IS NOT NULL 
                   THEN 'DONE'
                   ELSE null
            END AS FL_BIL
            from t_request a 
                        join t_request_cont b on a.ID = b.ID
                        join t_spk c on a.TGL_DOK = c.TGL_DOK and a.NO_DOK = c.NO_DOK 
                        ...
                        left join req_behandle_hdr z on a.NO_DOK = z.NO_DOK and a.TGL_DOK = z.TGL_DOK
                        left join t_log_kode_bayar_sap x on z.ID_REQ = x.PROFORMA
   ...
   //          where RIGHT(a.no_dok,6) like '%$nodok1%' and b.KD_STATUS = 'INQUIRY' and a.TGL_DOK = '$tgdok'")->result();
            ...
            from t_request a 
                        ...
                        left join req_behandle_hdr z on a.NO_DOK = z.NO_DOK and a.TGL_DOK = z.TGL_DOK
                        left join t_log_kode_bayar_sap x on z.ID_REQ = x.PROFORMA
            where a.no_dok like '%$nodok' and b.KD_STATUS = 'INQUIRY' and a.TGL_DOK = '$tgdok'")->result();
    ...
    {
        $no_cont = $this->input->post('cont');
        $no_dok = $this->input->post('nodok');

        $q = $this->db->query("SELECT A.*,C.NO_SPK,
            CASE WHEN LEFT(LOKASI,3) = 'CIC' THEN LOKASI WHEN LOKASI IS NULL THEN 'TERMINAL' ELSE CONCAT(LOKASI,'0', TIER) END AS 'LOKASI',
            X.TIPE, X.ISO_CODE, X.BRUTO
            FROM t_gatepass A
            ...
            WHERE A.NO_CONT = '$no_cont' AND A.NO_DOK = '$no_dok' AND A.JNS_KEGIATAN IN (1,2)
            ORDER BY A.ID DESC LIMIT 1")->row();

        if ($q) {
            return $this->httpres(200, 'success', $q->ID, '');
        } else {
