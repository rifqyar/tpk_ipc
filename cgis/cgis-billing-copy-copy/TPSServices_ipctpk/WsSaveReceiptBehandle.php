...
if (!$CheckFile) {
    ...
    if (date('H') >= $jam_awal && date('H') <= $jam_akhir) {
        $SQL = "SELECT DISTINCT A.ID AS ID, A.ID_REQ AS NO_ORDER, A.NO_NOTA_BEHANDLE AS NO_INVOICE, '' AS EX_NOTA, C.NAMA_BANK AS BANK,
                ...
                FROM req_behandle_hdr_ipctpk A                 
                ...
                WHERE 
                -- (A.FL_SEND_TRANS_SIMKEU = 'S' AND A.FL_SEND_RECEIPT_SIMKEU = 'N') AND A.NO_NOTA_BEHANDLE IS NOT NULL AND YEAR(A.TGL_NOTA) = '2018'
                      A.NO_NOTA_BEHANDLE = '010.801.18-65.008571'
		...
		$message .= '<source_system>BOSITPKCA</source_system>';
                ...
                $SQL = "SELECT A.JENIS_TARIF, A.CHARGE, COUNT(DISTINCT A.NO_CONT) AS QTY, SUM(A.TOTAL) AS TOTAL
                        FROM req_delivery_simkeu_ipctpk A INNER JOIN req_behandle_hdr B ON A.ID_REQ = B.ID_REQ
                        WHERE B.ID = '" . $data['ID'] . "'
...
//                echo "</pre>";
                ...
                if ($length > 1) {
                    for ($c = 0; $c < $length; $c++) {
                        ...
                        $SQL = "UPDATE req_behandle_hdr_ipctpk SET FL_SEND_RECEIPT_SIMKEU = '" . $status . "', 
