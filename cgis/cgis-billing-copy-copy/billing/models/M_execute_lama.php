...
class M_execute extends CI_Model {
		...
		if($type=="save"){
			if($act=="kapal"){
				...
				if ($cek_nota == NULL) {
					...
					$dataarr = array("NO_NOTA_DELIVERY" => $no_nota_delivery,"TGL_NOTA" =>date('Y-m-d H:i:s'),"BANK_ID" =>$split[0]);
	...
	function get_data($act,$id){
		...
		if($act=="kapal"){
			...
			$func->main->log_prints($id, $act);
			$arrid = explode("~",$id);
			$SQL="SELECT A.*, C.NO_SPK, CASE WHEN LEFT(B.LOKASI,3) = 'CIC' THEN B.LOKASI WHEN B.LOKASI IS NULL THEN 'TERMINAL' ELSE CONCAT(B.LOKASI,'0', B.TIER) END AS 'LOKASI', 
			E.KD_CONT_TIPE AS 'TIPE', E.ISO_CODE, D.NETTO, E.BRUTO, E.JNS_CONT, E.PEL_BONGKAR, 
			E.PEL_MUAT, F.NO_SEAL 
			FROM t_gatepass A 
			LEFT JOIN t_spk_cont B ON A.NO_CONT = B.NO_CONT 
			LEFT JOIN t_spk C ON A.NO_DOK = C.NO_DOK AND A.TGL_DOK = C.TGL_DOK 
