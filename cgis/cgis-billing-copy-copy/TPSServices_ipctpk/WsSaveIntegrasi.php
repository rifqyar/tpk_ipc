...
if (!$CheckFile) {
    ...
    if (date('H') >= $jam_awal && date('H') <= $jam_akhir) {
        $SQL = "SELECT DISTINCT A.ID AS ID, A.ID_REQ AS NO_ORDER, A.NO_NOTA_DELIVERY AS NO_INVOICE, A.ID_REQ_OLD AS EX_NOTA, C.NAMA_BANK AS BANK,
                ...
                WHERE -- A.FL_SEND_TRANS_SIMKEU = 'N' AND A.NO_NOTA_DELIVERY IS NOT NULL AND 
                     YEAR(A.TGL_NOTA) = '2018'
                     AND A.ID_REQ = 'EXT/2018-11-17/15507'
        ...
        if ($Query->size() > 0) {
            ...
            while ($Query->next()) {
                ...
                $SQL = "SELECT A.JENIS_TARIF, A.CHARGE, COUNT(DISTINCT A.NO_CONT) AS QTY, SUM(A.TOTAL) AS TOTAL
                        FROM req_delivery_simkeu A INNER JOIN req_delivery_hdr B ON A.ID_REQ = B.ID_REQ
                        WHERE B.ID = '" . $data['ID'] . "'
            ...
            if ($Send['response'] != '') {
                ...
                if ($length > 1) {
                    for ($c = 0; $c < $length; $c++) {
                        ...
                        $SQL = "UPDATE req_delivery_hdr SET FL_SEND_TRANS_SIMKEU = '" . $status . "', 
                        ...
                        $SQL = "UPDATE req_delivery_hdr SET FL_SEND_RECEIPT_SIMKEU = '" . $status . "', 
