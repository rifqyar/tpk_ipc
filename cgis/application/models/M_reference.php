...
class M_reference extends CI_Model {
	...
	function execute($type, $act, $id) {
        ...
        if ($type == "save") {
            if ($act == "libur") {
                ...
                $result = $this->db->insert('t_reff_alat', $DATA);
                if (!$result) {
                    $error += 1;
                    $message = "Data gagal diproses";
                }
                if ($error == 0) {
                    $func->main->get_log("add", "t_reff_alat");
                    echo "MSG#OK#Data berhasil diproses#" . $this->input->post('action');
                } else {
                    echo "MSG#ERR#" . $message . "#";
                }
            ...
            if ($act == "libur") {
                ...
                $this->db->where(array('ID' => $id));
                $result = $this->db->update('t_reff_alat', $DATA);
                if (!$result) {
                    $error += 1;
                    $message = "Data gagal diproses";
                }
                if ($error == 0) {
                    $func->main->get_log("update", "t_reff_alat");
                    echo "MSG#OK#Data berhasil diproses#" . $this->input->post('action');
                } else {
                    echo "MSG#ERR#" . $message . "#";
                }
            ...
            if ($act == "libur") {
                ...
                $arrid = explode("~", $id);
                $SQL = "SELECT A.* FROM t_reff_alat A
                        WHERE A.ID = " . $this->db->escape($arrid[0]);
                $result = $func->main->get_result($SQL);
                if ($result) {
                    foreach ($SQL->result_array() as $row => $value) {
            ...
            if ($act == "libur") {
                ...
                foreach ($this->input->post('tb_chktblalat') as $chkitem) {
                    $arrchk = explode("~", $chkitem);
                    $ID = $arrchk[0];
                    $result = $this->db->delete('t_reff_alat', array('ID' => $ID));
                    if (!$result) {
                        $error += 1;
                        $message .= "Could not be processed data";
                    }
                ...
                if ($error == 0) {
                    $func->main->get_log("delete", "t_reff_alat");
                    echo "MSG#OK#Successfully to be processed#" . site_url() . "/reference/alat/post#";
                } else {
                    echo "MSG#ERR#" . $message . "#";
                }
            }elseif($act == "truck"){
    ...
    public function alat($act, $id){
        $page_title = "ALAT PEKERJAAN";
        $title = "ALAT PEKERJAAN";
        $this->newtable->breadcrumb('Dashboard', site_url(),'icon-home');
        $this->newtable->breadcrumb('Reference', 'javascript:void(0)','');
        $this->newtable->breadcrumb('Alat', 'javascript:void(0)','');
        $check = (grant()=="W")?true:false;
        if($KD_GROUP!="SPA"){
            ///$addsql .= " AND A.KD_TPS = ".$this->db->escape($KD_TPS)." AND A.KD_GUDANG = ".$this->db->escape($KD_GUDANG);
        }
        $SQL ="SELECT ID, NM_ALAT AS 'NAMA ALAT', KEPEMILIKAN, JENIS_PEKERJAAN AS 'JENIS PEKERJAAN', DESCRIPTION FROM t_reff_alat ";

        $proses = array('ENTRY'  => array('MODAL',"reference/alat/add", '0','','md-plus-circle','', 'menu'),
                        'UPDATE' => array('MODAL',"reference/alat/update", '1','','md-edit','', 'list'),
                        'DELETE'  => array('DELETE',"reference/execute/delete/alat", 'all','','md-close-circle','', 'menu'));
        $this->newtable->multiple_search(true);
        $this->newtable->show_chk($check);
        $this->newtable->show_menu($check);
        $this->newtable->show_search(true);
        $this->newtable->search(array(array('NM_ALAT','NAMA ALAT'),array('KEPEMILIKAN','KEPEMILIKAN')));
        $this->newtable->action(site_url() . "/reference/alat");
        //if($check) $this->newtable->detail(array('DRILLDOWN',site_url()."/coarri/discharge/detail"));
        $this->newtable->tipe_proses('button');
        $this->newtable->hiddens(array("ID"));
        $this->newtable->keys(array("ID"));
        ...
        $this->newtable->orderby("ID");
        $this->newtable->sortby("ASC");
        $this->newtable->set_formid("tblalat");
        $this->newtable->set_divid("divalat");
        $this->newtable->rowcount(10);
        $this->newtable->clear();
        $this->newtable->menu($proses);
        $tabel .= $this->newtable->generate($SQL);
	...
	public function truck($act, $id){
        ...
        $SQL ="SELECT ID_TRUCK, NM_PEMILIK AS 'PEMILIK TRUCK',NO_TRUCK AS 'NO POLISI',UKURAN_TRUCK AS 'UKURAN', NO_PLAT AS 'NO PLAT', BERAT_TRUCK AS 'BERAT TRUCK' FROM reff_truck ";
