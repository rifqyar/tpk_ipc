...
if (!$CheckFile) {
    ...
    if (date('H') >= $jam_awal && date('H') <= $jam_akhir) {
        $SQL = "SELECT DISTINCT A.ID AS ID, A.ID_REQ AS NO_ORDER, A.NO_NOTA_DELIVERY AS NO_INVOICE, A.ID_REQ_OLD AS EX_NOTA, C.NAMA_BANK AS BANK,
                       E.CONSIGNEE AS NAMA, A.NPWP AS NPWP, A.SUBTOTAL, A.PPN, A.TOTAL, DATE_FORMAT(A.TGL_NOTA,'%d/%m/%Y %H:%i:%s') AS TGL_TERIMA, 
                       A.NM_KAPAL AS NM_ANGKUT, A.NO_VOY AS NO_VOYAGE, DATE_FORMAT(E.TGL_BC11,'%d/%m/%Y') AS TGL_TIBA, A.NO_DO AS NO_DO, A.NO_BL AS NO_BL_AWB, 
                       DATE_FORMAT(A.EXPIRED,'%d/%m/%Y') AS TGL_KELUAR, A.FL_SEND_TRANS_SIMKEU, A.FL_SEND_RECEIPT_SIMKEU , F.ID_CUST AS 'ID_CUST'
                FROM req_delivery_hdr_ipctpk A INNER JOIN m_bank C ON A.BANK_ID = C.BANK_ID
                                        INNER JOIN req_delivery_simkeu_ipctpk D ON A.ID_REQ = D.ID_REQ
                ...
                /* WHERE  A.NO_NOTA_DELIVERY IN ('030.801.18-61.010462','030.801.18-61.008444','030.801.18-61.008472','010.801.18-61.009402','010.801.18-61.009536','030.801.18-61.009916','010.801.18-61.011227')
                    AND A.ID_REQ IN ('DEL/2018-10-11/13959','DEL/2018-08-17/11680','EXT/2018-08-20/11715','DEL/2018-09-14/12800','DEL/2018-09-19/12948','EXT/2018-09-19/12947')
                */
                WHERE A.NO_NOTA_DELIVERY IN ('031.801.18-61.012236')
                GROUP BY A.NO_NOTA_DELIVERY
                ORDER BY A.NO_NOTA_DELIVERY ASC
                LIMIT 50";
                echo $SQL;
        ...
        if ($Query->size() > 0) {
            ...
            while ($Query->next()) {
                ...
                $is_transaction = $data['FL_SEND_TRANS_SIMKEU'] == "Y" ? "F" : "T";
                $is_receipt = $data['FL_SEND_RECEIPT_SIMKEU'] == "Y" ? "F" : "T";
                $is_payable = "F";
                //$dataBank['BANK_ID'] = '124008';
                //$dataBank['BANK_ACCOUNT'] = '120002200009';

                $message .= '<component>';
                $message .= '<transaction>';
                $message .= '<header>';
                $message .= '<transaction_number>' . $data['NO_INVOICE'] . '</transaction_number>';
                $message .= '<prev_transaction_number>' . $data['EX_NOTA'] . '</prev_transaction_number>';
                $message .= '<request_number>' . $data['NO_ORDER'] . '</request_number>';
                $message .= '<tax_number>' . $data['NO_INVOICE'] . '</tax_number>';
                $message .= '<header_context>PTKM</header_context>'; //DARI MAS PANDIT
                $message .= '<header_sub_context>PTKM01</header_sub_context>'; //DARI MAS PANDIT
                $message .= '<organization_id>1827</organization_id>';
                $message .= '<transaction_date>' . $data['TGL_TERIMA'] . '</transaction_date>';
                $message .= '<transaction_type>PELAYANAN JASA DELIVERY</transaction_type>';
                $message .= '<customer_number>999999</customer_number>';
                $message .= '<customer_name>CUSTOMER PEMILIK BARANG</customer_name>';
                $message .= '<no_do>' . $data['NO_DO'] . '</no_do>';
                $message .= '<no_bl>' . $data['NO_BL_AWB'] . '</no_bl>';
                $message .= '<vessel_name>' . $data['NM_ANGKUT'] . '</vessel_name>';
                $message .= '<arrival_date>' . $data['TGL_TIBA'] . '</arrival_date>';
                $message .= '<location_code>ITPK</location_code>';
                $message .= '<location>IPCTPK COMMON AREA</location>';
                $message .= '<delivery_date>' . $data['TGL_KELUAR'] . '</delivery_date>';
                ...
                $message .= '<before_tax>' . $data['SUBTOTAL'] . '</before_tax>';
                $message .= '<tax>' . $data['PPN'] . '</tax>';
                $message .= '<total>' . $data['TOTAL'] . '</total>';
                $message .= '<source_system>BOSITPKCA</source_system>';
                $message .= '<operation_unit_code>ICMA</operation_unit_code>';
                ...
                $message .= '<atribute2>' . $data['ID_CUST'] . '</atribute2>';
                $message .= '<atribute3>' . $data['NAMA'] . '</atribute3>';
                $message .= '<atribute5>' . $data['NPWP'] . '</atribute5>';
                $message .= '</header>';

                $SQL = "SELECT A.JENIS_TARIF, A.CHARGE, COUNT(DISTINCT A.NO_CONT) AS QTY, SUM(A.TOTAL) AS TOTAL
                        FROM req_delivery_simkeu_ipctpk A INNER JOIN req_delivery_hdr_ipctpk B ON A.ID_REQ = B.ID_REQ
                        WHERE B.ID = '" . $data['ID'] . "'
                ...
                if ($Query2->size() > 0) {
                    ...
                    while ($Query2->next()) {
                        ...
                        if ($data2['JENIS_TARIF'] == 'ADMINISTRASI') {
                            $serviceType = 'CMA DELIVERY ADM';
                            $data2['QTY'] = '1';
                        } else if ($data2['JENIS_TARIF'] == 'BEHANDLE 1 ADMINISTRASI') {
                            $serviceType = 'CMA BEHANDLE ADM';
                            $data2['QTY'] = '1';
                        } else if ($data2['JENIS_TARIF'] == 'BEHANDLE 2 ADMINISTRASI') {
                            $serviceType = 'CMA BEHANDLE ADM';
                            $data2['QTY'] = '1';
                            ...
                            $serviceType = 'CMA BEHANDLE BEHANDLE';
                            $data2['QTY'] = '1';
                        } else if ($data2['JENIS_TARIF'] == 'PENUMPUKAN 1') {
                            $serviceType = 'CMA DELIVERY PNP_M_1-1';
                            $data2['QTY'] = '1';
                        } else if ($data2['JENIS_TARIF'] == 'PENUMPUKAN 1.1') {
                            $serviceType = 'CMA DELIVERY PNP_M_1-2';
                            $data2['QTY'] = '1';
                        } else if ($data2['JENIS_TARIF'] == 'PENUMPUKAN 2') {
                            $serviceType = 'CMA DELIVERY PNP_M_2';
                            $data2['QTY'] = '1';
                        } else if ($data2['JENIS_TARIF'] == 'PENUMPUKAN NHI') {
                            $serviceType = 'CMA DELIVERY PNP_NHI';
                            $data2['QTY'] = '1';
                        } else if ($data2['JENIS_TARIF'] == 'DENDA SPPB 2') {
                            $serviceType = 'CMA DELIVERY SPPB';
                            $data2['QTY'] = '1';
                        } else if ($data2['JENIS_TARIF'] == 'DENDA SP 2') {
                            $serviceType = 'CMA DELIVERY SP2';
                            $data2['QTY'] = '1';
                        } else if ($data2['JENIS_TARIF'] == 'MATERAI DEL') {
                            $serviceType = 'CMA DELIVERY MATERAI';
                            $data2['QTY'] = '1';
                        } else if ($data2['JENIS_TARIF'] == 'LIFT ON') {
                            $serviceType = 'CMA DELIVERY LIFT_ON';
                            $data2['QTY'] = '1';
                        } else {
                            $serviceType = "";
                        }
                        ...
                        if ($data2['JENIS_TARIF'] == 'MATERAI DEL') {
                ...
                $message .= '<receipt_sub_type>PTKM01</receipt_sub_type>';
                $message .= '</receipt>';
                $message .= '</component>';
            }
            $message .= '</group>';
            ...
            if ($Send['response'] != '') {
                ...
                if ($length > 1) {
                    for ($c = 0; $c < $length; $c++) {
                        ...
                        //$status = strtoupper($transaction['status']['_v']) == "S" ? "Y" : "E";
                        $status = strtoupper($transaction['status']['_v']);
                        $message = $transaction['message']['_v'];
                        $SQL = "UPDATE req_delivery_hdr_ipctpk SET FL_SEND_TRANS_SIMKEU = '" . $status . "', 
                                                            MESSAGE_SEND_TRANS_SIMKEU = '" . $message . "', 
                                                            TGL_SEND_TRANS_SIMKEU = NOW()
                                WHERE NO_NOTA_DELIVERY = '" . $transaction_number . "'";
                        echo $SQL . "<br>";
                        $Execute = $conn->execute($SQL);

                        $receipt = $component[$c]['_c']['receipt']['_c'];
                        $receipt_number = $receipt['receipt_number']['_v'];
                        //$status = strtoupper($receipt['status']['_v']) == "S" ? "Y" : "E";
                        $status = strtoupper($receipt['status']['_v']);
                        $message = $receipt['message']['_v'];
                        $SQL = "UPDATE req_delivery_hdr_ipctpk SET FL_SEND_RECEIPT_SIMKEU = '" . $status . "', 
                                                            MESSAGE_SEND_RECEIPT_SIMKEU = '" . $message . "', 
                                                            TGL_SEND_RECEIPT_SIMKEU = NOW()
                                WHERE NO_NOTA_DELIVERY = '" . $receipt_number . "'";
                        echo $SQL . "<br>";
                        $Execute = $conn->execute($SQL);
                        ...
                        /* BELUM DIPAKE
                          ...
                          $status = strtoupper($payable['status']['_v']) == "S" ? "Y" : "E";
