<?php 
	...
	* 
	*/
	class Dokumenmanual extends CI_Controller
	{
		...
		public function __construct() {
	        parent::__construct();
			$this->load->model("m_dokumen_manual");
	    }
		...
		public function cetak(){
			...
			//echo $id." - ".$nocont;
			$idpost = $id.'~'.$nocont;
			$arrdata = $this->m_dokumen_manual->execute($type,$act,$idpost);
		...
		public function gatepass($act, $id){
			...
			if($act=="gate_pass_request_detail"){
				...
				$data['id'] = $id;
				$this->load->model("m_dokumen_manual");
				$data['arrdata'] = $this->m_dokumen_manual->get_data('gate_pass_request_detail', $id);
				echo $this->content = $this->load->view('content/dokumen/gate_pass_detail',$data,true);
			}else if($act == "add_keterangan"){
				$type = 'update';
				$act = 'fl_finish';
				$arrdata = $this->m_dokumen_manual->execute($type,$act, $id);
			}else if($act == "cetak"){
				$type = 'update';
				$act = 'fl_finish_print';
				$arrdata = $this->m_dokumen_manual->execute($type,$act, $id);
				...
				$arrdata = $this->m_dokumen_manual->execute($type,$act, $arrdid);
				//echo $this->content = $this->load->view('content/dokumen/form_paidthrought',$data);
			}else{
				$this->load->model("m_dokumen_manual");
				$arrdata = $this->m_dokumen_manual->list_reqGatePassKarantina($act, $id);
		...
		public function gatepassBc($act, $id){
			...
			if($act=="gate_pass_request_detail"){
				...
				$this->load->model("m_dokumen_manual");
				$arrdata = $this->m_dokumen_manual->list_reqGatePassBc($act, $id);
				$data = $this->load->view('content/newtable', $arrdata, true);
				if($this->input->post("ajax")||$act=="post"){
					echo $arrdata;
				}else{
		...
		public function gatepassJoin($act, $id){
			...
			if($act=="gate_pass_request_detail"){
				...
				$data['id'] = $id;
				$this->load->model("m_dokumen_manual");
				$data['arrdata'] = $this->m_dokumen_manual->get_data('gate_pass_request_detailjoin', $id);
				echo $this->content = $this->load->view('content/dokumen/gate_pass_detail',$data,true);
			}else if($act == "add_keterangan"){
				$type = 'update';
				$act = 'fl_finish';
				...
				$this->load->model("m_dokumen_manual");
				$arrdata = $this->m_dokumen_manual->list_reqGatePassJoin($act, $id);
				$data = $this->load->view('content/newtable', $arrdata, true);
				if($this->input->post("ajax")||$act=="post"){
					echo $arrdata;
				}else{
		...
		function ondemand($act, $id){
			...
			if ($act=="ondimine_gatepass") {
				...
				$this->load->model("m_dokumen_manual");
				$data['arrdata'] = $this->m_dokumen_manual->execute($type, $act, $id);
			} else {
				$this->load->model("m_dokumen_manual");
				$arrdata = $this->m_dokumen_manual->list_gatepassondemand($act, $id);
				$data = $this->load->view('content/newtable', $arrdata, true);
				if($this->input->post("ajax")||$act=="post"){
					return $arrdata;
				}else{
		...
		function ondemand_api($act, $id){
			...
			if ($act=="ondimine_gatepass_api") {
				...
				$this->load->model("m_dokumen_manual");
				$arrdata = $this->m_dokumen_manual->list_gatepassondemand_api($act, $id);
				$data = $this->load->view('content/newtable', $arrdata, true);
				if($this->input->post("ajax")||$act=="post"){
					return $arrdata;
				}else{
		...
		public function request($type="", $act="", $id="", $met=""){
			...
			$id = ($id!="")?$id:$this->input->post('id');
			$this->load->model('m_dokumen_manual');
			$this->m_dokumen_manual->execute($type, $act, $id);
		}
		...
		function proces($type="",$act="", $id=""){
			if (!$this->session->userdata('LOGGED')) {
				...
				if (strtolower($_SERVER['REQUEST_METHOD']) != "post") {
					$this->load->model("m_dokumen_manual");
					$this->m_dokumen_manual->process($type,$act,$id);
				}else{
					$this->load->model("m_dokumen_manual");
					$this->m_dokumen_manual->process($type,$act,$id);
				}
