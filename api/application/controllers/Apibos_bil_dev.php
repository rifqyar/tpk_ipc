...
class Apibos_bil_dev extends CI_Controller {
	...
	public function syncdok_dll()
	{
		...
		try {
            ...
            CASE WHEN x.SAP_TGL_PELUNASAN IS NOT NULL 
                   ...
                   ELSE null
            END AS FL_BIL,
            case when tgt.NO_CONT is not null
            		then 'DONE'
            		else null
            end as FL_BIL_DEL,
            f.STATUS as 'STATUS_PERIKSA'
            from t_request a 
                        join t_request_cont b on a.ID = b.ID
                        ...
                        left join t_gatepass_temp tgt on c.NO_SPK = tgt.NO_SPK
            where a.no_dok = '$nodok' and b.KD_STATUS = 'INQUIRY' and a.TGL_DOK = '$tgdok'")->result();
			if ($q) {
				$this->httpres(200,'success',$q,'');
			}else {
				$this->httpres(200,'error','','Data Tidak Di Temukan');
