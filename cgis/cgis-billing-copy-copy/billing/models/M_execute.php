...
class M_execute extends CI_Model {
	...
	public function emailnya($alamatemail,$nama,$pass){
		...
		if(valid_email($email)){
			...
			$this->email->from('cgs.ipc@gmail.com', 'BOS NOTIFICATION - CUSTOMER REGRISTRATION');
		...
		if($type=="save"){
				...
				if ($IDHEADER != "") {
					...
					if ($banyakcont > 0) {
						...
						$addXml .= "<LOOP>
										...
										<SP3UDKTG>".$KONTAINER["TGL_TPFT"]."</SP3UDKTG>
									</LOOP>";

								// PRINT_R($this->input->post('KONTAINER')); die();
					...
					$addXml .= "<DOKUMEN>
									<LOOP>
									<NO_DOK>MSCUZ7902464</NO_DOK>
									<TG_DOK>20170512</TG_DOK>
									</LOOP>
								</DOKUMEN>
					</PERSETUJUAN>";
					$dataArr = array(
			...
			if($act=="kapal"){
				...
				foreach($this->input->post('DATA') as $a => $b){
					...
					$ID = $DATA['ID'];
					$DATAE['NAMA_CUST'] = $DATA['NAMA_CUST'];
					$DATAE['NO_VOY'] = $DATA['NO_VOY'];
					$DATAE['NM_KAPAL'] = $DATA['NM_KAPAL'];
					$DATAE['NPWP'] = $DATA['NPWP'];
					//print_r($DATA); die();
					$this->db->where(array('ID' => $ID));
					$result = $this->db->update('t_gatepass', $DATAE);
				...
				$DATA_GP['TGL_DOK'] = $GTGL;
				$DATA_GP['NO_DOK'] = $DATA['NO_DOK'];
				$DATA_GP['JNS_DOK'] = $DATA['JNS_DOK'];
				$DATA_GP['NO_CONT'] = $DATA['NO_CONT'];
				$DATA_GP['NO_SPK'] = $DATA['NO_SPK'];

				$DOK1 = $this->input->post('DOK');
				$DATAT['FL_USE'] = 'Y';
				//print_r($DATA_GP);die();

				$this->db->insert('t_gatepass', $DATA_GP);

				$this->db->where(array('NO_DOK' => $DOK1),array('JNS_KEGIATAN' => '1'));
	...
	function get_data($act,$id){
		...
		if($act=="kapal"){
			...
			$SQL = "SELECT X.ID,X.NAMA_KAPAL AS 'NAMA KAPAL',X.NO_VOYAGE AS 'NO. VOYAGE', X.JENIS_DOKUMEN AS 'JENIS DOKUMEN', X.NO_DOKUMEN AS 'NO. DOKUMEN',DATE_FORMAT(X.TGL_DOKUMEN,'%d-%m-%Y') AS 'TGL. DOKUMEN', X.NO_KONTAINER AS 'NO. KONTAINER', X.UKURAN AS 'UKURAN', X.STATUS_DOKUMEN AS 'STATUS DOKUMEN'
					FROM(
					SELECT C.ID AS 'ID', C.ANGKUTNAMA_TPS AS 'NAMA_KAPAL', C.ANGKUTNO_TPS 'NO_VOYAGE', H.NAMA AS 'JENIS_DOKUMEN', C.NO_DAFTAR_PABEAN AS 'NO_DOKUMEN', C.TGL_DAFTAR_PABEAN AS 'TGL_DOKUMEN',D.NO_CONT AS 'NO_KONTAINER', KD_CONT_UKURAN AS 'UKURAN', CASE WHEN C.FL_MANUAL IS NULL THEN 'INTEGRASI' ELSE 'MANUAL' END AS 'STATUS_DOKUMEN'
					FROM t_permit_hdr C
			...
			//echo $jns_dokumen;
			$query = "SELECT NO_SPK FROM t_spk WHERE NO_DOK = '$no_dok' AND JNS_DOK = '$jns_dokumen'";
			...
			$SQL="SELECT A.*,C.NO_SPK, CASE WHEN
					LEFT(LOKASI,3) = 'CIC' THEN LOKASI WHEN LOKASI IS NULL THEN 'TERMINAL' ELSE CONCAT(LOKASI,'0', TIER) END AS 'LOKASI', X.TIPE, X.ISO_CODE, X.BRUTO
					FROM t_gatepass A
			...
			E.KD_CONT_TIPE AS 'TIPE', E.ISO_CODE, D.NETTO, E.BRUTO, E.JNS_CONT, E.PEL_BONGKAR, E.PEL_MUAT, H.NO_SEAL 
			FROM t_gatepass A 
			LEFT JOIN t_spk_cont B ON A.NO_CONT = B.NO_CONT 
			LEFT JOIN t_spk C ON A.NO_DOK = C.NO_DOK AND A.TGL_DOK = C.TGL_DOK 
			...
			LEFT JOIN t_op_inspection H ON A.NO_CONT = H.NO_CONT AND A.NO_DOK = H.NO_DOK
	...
	public function get_data_dokumen($act, $id){
		if ($act == 'spjm') {
			...
			$sql = $this->db->query("SELECT * FROM t_permit_cont WHERE KD_STATUS_BIL IS NULL AND ID = '$id'");
			return $sql->result();
		}else if($act == 'spk'){
	...
	function cek_data($act,$id){
		...
		if($act == "active_gatepass"){
			...
			if($error == 0){
				$action = '/planning/get_pass_behand/post';
				echo "MSG#OK#Data berhasil diproses#".site_url().$action;
			}else{
				echo "MSG#ERR#".$message."#";
			}
			...
			if($error == 0){
				$action = '/display/respon_custom/post';
				echo "MSG#OK#Data berhasil diproses#".site_url() . "/display/respon_custom/post";
			}else{
				echo "MSG#ERR#".$message."#";
			}
