...
class BillingDeliveryExt extends CI_Controller {

	public function __construct()
	{
		parent::__construct();
		$this->load->model("m_billing_delivery_ext");
	}
	...
	public function delivery_ext($act, $id){
		...
		$arrdata = $this->m_billing_delivery_ext->delivery_ext($act, $id);
	...
	public function detail_sppb_ext($act, $id){
		...
		$id = ($id!="")?$id:$this->input->post('id');
		$arrdata = $this->m_billing_delivery_ext->delivery_ext_add($act, $id);
		$data = $this->load->view('content/newtable', $arrdata, true);
		if($this->input->post("ajax")||$act=="post"){
			echo $arrdata;
		}else{
	...
	public function save_delivery($act, $id){
		...
		$data['id'] = $id;
		$data['action'] = "save";
		$data['arrdata'] = $this->m_billing_delivery_ext->get_data_delivery_hdr($id);
		$data['detail_cont'] = $this->m_billing_delivery_ext->get_data_delivery_dtl($id);
		$this->load->view('content/billing/delivery_ext/form_add', $data);
	}

	public function cetak_nota_ext($id){
		if (!$this->session->userdata('LOGGED')) {
            $this->index();
            return;
        }
		...
		$data['title'] = "".$sess; 
		$data['result'] = $this->m_billing_delivery_ext->get_nota_del($id); 
		$data['result_hdr'] = $this->m_billing_delivery_ext->get_nota_del_hdr($id);
		$data['result_cust'] = $this->m_billing_delivery_ext->get_nota_cust($id);
		$data['result_cont'] = $this->m_billing_delivery_ext->get_nota_cont($id);
	...
	public function process($act="", $id=""){
		...
		if (!$this->session->userdata('LOGGED')) {
			...
			if (strtolower($_SERVER['REQUEST_METHOD']) != "post") {
				...
				$this->m_billing_delivery_ext->process($act,$id);
			}
		}
	}

	public function delivery_ext_confirm($act,$id){
		if (!$this->session->userdata('LOGGED')){
			$this->index();
			return;
		}
		...
		$data['bank'] = $this->m_billing_delivery_ext->get_bank_data();
		echo $this->content = $this->load->view('content/billing/delivery_ext/confirm',$data,true);
	}

	public function confirm_ext($act="", $id=""){
		$id = ($id!="")?$id:$this->input->post('id');
		if (!$this->session->userdata('LOGGED')) {
			$this->index();
			return;
			...
			if (strtolower($_SERVER['REQUEST_METHOD']) != "post") {
				...
				$this->m_billing_delivery_ext->confirm_delivery_ext($act,$id);
			}
		}
	}

	public function delivery_ext_history($act, $id){
		if (!$this->session->userdata('LOGGED')) {
            $this->index();
            return;
        } 
        $arrid = explode('~',$this->input->post('id'));
		$id = $arrid[0];
		$data['arrdata'] = $this->m_billing_delivery_ext->get_history_deliveryext($id);

		echo $this->load->view('content/dokumen/history_cetak',$data,true);
	}

	public function delivery_ext_update($act, $id){
		if (!$this->session->userdata('LOGGED')){
			$this->index();
			return;
		}
		...
		$data['id'] = $id;
		$data['action'] = "update";
		$data['arrdata'] = $this->m_billing_delivery_ext->get_data_dokumen_delivery($id);
		//print_r($data);die();
		$this->load->view('content/billing/delivery_ext/delivery', $data);
	}
	
	public function delivery_ext_del($act, $id){
		if (!$this->session->userdata('LOGGED')){
			$this->index();
			return;
		}
		$arrpost = $this->input->post('tb_chktbldeliveryext');
		$exparrpost = explode('~',$arrpost[0]);
		$id_req = $exparrpost[0];
		$no_dok = $exparrpost[1];
		$no_nota = $exparrpost[2];
		$key = $id_req."~".$no_dok."~".$no_nota;
		$this->m_billing_delivery_ext->del_delivery_ext($key);
	}

	public function delivery_ext_resend($id){
		if (!$this->session->userdata('LOGGED')){
			$this->index();
			return;
		}
		foreach ($this->input->post('tb_chktbldeliveryext') as $CheckArr) {
			...
			$this->m_billing_delivery_ext->resend_deliveryext($IdReq, $FlTrans, $FlReceipt);
		}
	}
}

/* End of file BillingDeliveryExt.php */
/* Location: ./application/controllers/BillingDeliveryExt.php */