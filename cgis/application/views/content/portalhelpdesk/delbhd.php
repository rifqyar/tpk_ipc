...
<div class="page-heading">
    <h3>Hapus Status Billing Behandle</h3>
</div>
    ...
    <div class="container mt-5">
        <div class="row">
            <div class="col-md-12">
                <form id="searchForm">
                    <div class="mb-3">
                        <label for="nomorDokumen" class="form-label mb-3">Nomor Dokumen Behandle</label>
    ...
    <script>
        $(document).ready(function () {
            $('#searchForm').submit(function (e) {
                ...
                $.ajax({
                    url: "<?php echo base_url('application.php/PortalHelpdesk/getbehandle'); ?>", // Add application.php to base_url
                    method: 'POST',
                        ...
                        if (response.length > 0) {
                            ...
                            $.each(response, function (index, data) {
                                var row = '<tr>' +
                                    ...
                                    '<td>' + data.TGL_DOK + '</td>' +
                                    '<td>' + data.NO_CONT + '</td>' +
                                    '<td>' + (data.FL_BIL ? data.FL_BIL : '-') + '</td>' +
                                    '<td><a href="#" class="delete-link" data-id="' + data.ID + '" data-tgl="' + data.TGL_DOK + '" data-no-dok="' + data.NO_DOK + '">Hapus Data</a></td>' +
                                    '</tr>';
        ...
        $(document).on('click', '.delete-link', function (e) {
            ...
            var noDok = $(this).data('no-dok'); 

            // Show confirmation alert
            if (confirm('Apakah anda yakin untuk menghapus status billing ini?')) {
                // Send AJAX request to delete the data
                $.ajax({
                    url: "<?php echo base_url('application.php/PortalHelpdesk/prosesbhd'); ?>", // The delete URL
                    method: 'POST',
                    data: { tgl: tgl, no_dok: noDok }, // Include NO_DOK_INOUT
                    success: function (response) {
